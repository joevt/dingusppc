...
static const char * get_sub_name(int32_t x) {
    switch (x) {
        #define oneevent(x) case x: return #x ;
        oneevent(SDL_WINDOWEVENT_NONE)
        oneevent(SDL_WINDOWEVENT_SHOWN)
        oneevent(SDL_WINDOWEVENT_HIDDEN)
        oneevent(SDL_WINDOWEVENT_EXPOSED)
        oneevent(SDL_WINDOWEVENT_MOVED)
        oneevent(SDL_WINDOWEVENT_RESIZED)
        oneevent(SDL_WINDOWEVENT_SIZE_CHANGED)
        oneevent(SDL_WINDOWEVENT_MINIMIZED)
        oneevent(SDL_WINDOWEVENT_MAXIMIZED)
        oneevent(SDL_WINDOWEVENT_RESTORED)
        oneevent(SDL_WINDOWEVENT_ENTER)
        oneevent(SDL_WINDOWEVENT_LEAVE)
        oneevent(SDL_WINDOWEVENT_FOCUS_GAINED)
        oneevent(SDL_WINDOWEVENT_FOCUS_LOST)
        oneevent(SDL_WINDOWEVENT_CLOSE)
        oneevent(SDL_WINDOWEVENT_TAKE_FOCUS)
        oneevent(SDL_WINDOWEVENT_HIT_TEST)
        oneevent(SDL_WINDOWEVENT_ICCPROF_CHANGED)
        oneevent(SDL_WINDOWEVENT_DISPLAY_CHANGED)
        #undef oneevent
        default: return "unknown";
    }
}
...
void EventManager::poll_events() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_WINDOWEVENT: {
                LOG_F(HOSTEVENTS, "event: %d = %s , %d = %s",
                    event.type, get_event_name(event.type),
                    event.window.event, get_sub_name(event.window.event)
                );
                WindowEvent we{};
