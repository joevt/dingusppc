...
int main(int argc, char** argv) {
    ...
    emu->add_option("--setenv", OfConfigUtils::env_vars, "Set Open Firmware variables at startup")
        ->take_all();
    emu->add_option("--patch", BootRom::rom_patches, "Patch boot ROM")
        ->take_all();
    emu->add_option("--setnvram", NVram::nvram_patches, "Patch NVRAM")
        ->take_all();
    ...
    while (true) {
        ...
        run_machine(execution_mode, deterministic_interactive, profiling_interval_ms);
...
void run_machine(
    ...
    static TimerInfo deterministic_timer;
    if (is_deterministic && !deterministic_interactive) {
        ...
        TimerManager::get_instance()->add_cyclic_timer(deterministic_timer, MSECS_TO_NSECS(1000), [](uint64_t, uint64_t) {
    ...
    static TimerInfo event_timer;
    TimerManager::get_instance()->add_cyclic_timer(event_timer, MSECS_TO_NSECS(11), [](uint64_t, uint64_t) {
