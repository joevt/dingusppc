...
#define assert_size(typ, siz) \
int size_ ## typ[-(sizeof(typ) != siz)]
...
typedef uint32_t    vmmCntrlTable_p;
 ...
 typedef struct {
    ...
    uint64_t            qactTimer __attribute__((packed));
    uint32_t            ksp;
    uint32_t            bbDescAddr;
    uint32_t            bbUserDA;
...
typedef struct {
    ...
    uint64_t            deadline __attribute__((packed));
    uint32_t            state;
} timer_call_data_t;
...
typedef struct {
    ...
    event64_t           wait_event __attribute__((packed));
...
# error "Please fix <bits/endian.h>"
    ...
    uint64_t            last_switch __attribute__((packed));
    uint64_t            computation_metered __attribute__((packed));
    uint64_t            computation_epoch __attribute__((packed));
    integer_t           safe_mode;
...
typedef struct {
    ...
    vm_object_offset_t  offset __attribute__((packed));
    union {
        struct {
#if __BYTE_ORDER == __LITTLE_ENDIAN
...
typedef struct {
    ...
    timeval_32          p_rtime;
    u_quad_t            p_uticks __attribute__((packed));
    u_quad_t            p_sticks __attribute__((packed));
    u_quad_t            p_iticks __attribute__((packed));
    int32_t             p_traceflag;
    vnode_p             p_tracep;
