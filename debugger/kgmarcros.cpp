...
uint64_t kernel_read(uint32_t addr, uint32_t size) {
    uint8_t* kernel_mem = get_kernel_mem();
    if (kernel_mem) {
        switch (size) {
            case 1: return  *((uint8_t  *)(kernel_mem + addr));
            case 2: return READ_WORD_BE_A (kernel_mem + addr);
            case 4: return READ_DWORD_BE_A(kernel_mem + addr);
            case 8: return READ_QWORD_BE_A(kernel_mem + addr);
            default: return 0;
        }
    }
    else {
...
void kernel_write(uint32_t addr, uint64_t val, uint32_t size) {
    ...
    if (kernel_mem) {
        switch (size) {
            case 1: *((uint8_t  *)(kernel_mem + addr)) = (uint8_t )val;
            case 2: WRITE_WORD_BE_A (kernel_mem + addr, (uint16_t)val);
            case 4: WRITE_DWORD_BE_A(kernel_mem + addr, (uint32_t)val);
            case 8: WRITE_QWORD_BE_A(kernel_mem + addr, (uint64_t)val);
        }
    }
}
...
#define readfield(fld) \
    do { \
        try { \
            s.fld = (std::remove_reference_t<decltype((s.fld))>)kernel_read(p + (uint32_t)((char*)&s.fld - (char*)&s), sizeof(s.fld)); \
        } catch (...) { \
            s.fld = 0; \
        } \
    } while (0)
...
#define readfieldvm(fld) \
    do { \
        try { \
            s.fld = (std::remove_reference_t<decltype((s.fld))>)mem_read_dbg(p + (uint32_t)((char*)&s.fld - (char*)&s), sizeof(s.fld)); \
        } catch (...) { \
            s.fld = 0; \
        } \
    } while (0)
...
static void get_kmod_info(uint32_t p, kmod_info_t &s) {
    ...
    readfieldvm(start          );
    readfieldvm(stop           );
    for (int i = 0; i < 8; i++) { val = mem_read_dbg(p + offsetof(kmod_info_t, name   ) + i * 8, 8); WRITE_QWORD_BE_A(&(((uint64_t*)(&s.name   ))[i]), val); if (!val) break; };
    for (int i = 0; i < 8; i++) { val = mem_read_dbg(p + offsetof(kmod_info_t, version) + i * 8, 8); WRITE_QWORD_BE_A(&(((uint64_t*)(&s.version))[i]), val); if (!val) break; };
}
...
void showallacts() {
    ...
    int x = sizeof(((queue_head_t*)(0))->next);
...
static void showwaitqmemberof(uint32_t arg0) {
    ...
    while ((queue_entry_t)kgm_mo_wqe != (queue_entry_t)kgm_mo_linksp) {
        if (get_fieldvm(wait_queue_element, kgm_mo_wqe, wqe_type) == _wait_queue_link) {
                ...
                uint32_t kgm_mo_wqsetp = (uint32_t)get_fieldvm(wait_queue_link, kgm_mo_wqlp, wql_setqueue); // (struct wait_queue *)
...
static void showpsetint(uint32_t arg0) {
    ...
    while ((queue_entry_t)kgm_wql != (queue_entry_t)kgm_setlinksp) {
        uint32_t kgm_portp = ((int)(get_fieldvm(wait_queue_link, kgm_wql, wql_element.wqe_queue)) - ((int)kgm_portoff)); // (struct ipc_port *)
        if (!kgm_found) {
            kgm_destspacep = 0; // (struct ipc_space *)
            showportdestproc(kgm_portp);
