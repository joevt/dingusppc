...
std::string get_name_OpenFirmware(uint32_t addr, uint32_t addr_p, int *offset, bool append_offset) {
    ...
    do {
        ...
        // Usually start vector has a negative address but it is sometimes positive when it is real instead of virtual.
        if (start_vector_ptr == 0 || ((int32_t)start_vector_ptr > 0 && start_vector_ptr != start_vector_ptr_saved_p))
            break;
        ...
        if (addr == addr_p) {
            ...
            if (start_vector_ptr != start_vector_ptr_saved && start_vector_ptr != start_vector_ptr_saved_p)
                break;

            // Perform a virtual to physical translation of here.
            here = here - start_vector_ptr_saved + start_vector_ptr_saved_p;
        ...
        if (start_vector_ptr <= addr && addr <= here) {
            ...
            while (begin_addr >= start_vector_ptr) {
                the_addr = begin_addr;
                this_page = the_addr & PPC_PAGE_MASK;
                if (this_page != virt_page) {
                    virt_page = this_page;
                    try {
                        host_page = mmu_translate_imem(virt_page);
                    } catch (invalid_argument& exc) {
                        cout << "mmu_translate_imem caused exception: " << exc.what() << endl;
                        break;
                    }
                }
                val = READ_QWORD_BE_A(&host_page[the_addr & ~PPC_PAGE_MASK]);
                h_link = val >> 32;
                prev_begin_addr = 0;
