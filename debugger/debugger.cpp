...
static std::string get_name(uint32_t addr, uint32_t addr_p) {
    std::string str;
                ...
                if (h_link == 0 ||
                    ...
                    if (h_flags & fdefd) {
                        ...
                        if ((h_ctype >= ctype_colon && h_ctype <= ctype_code) || h_ctype == ctype_settoken) {
                            ...
                            char offset[20];
                            if (addr != begin_addr) {
                                snprintf(offset, sizeof(offset), "%+-6d", int32_t(addr - begin_addr));
                                str += offset;
                            }
                            else {
                                str += "      ";
...
static uint32_t disasm(PPCDisasmContext &ctx) {
    ...
    std::string name = get_name(ctx.instr_addr, phys_addr);
    if (!name.empty()) {
...
static void print_mmu_regs()
{
    ...
    for (int i = 0; i < 4 + (is_601 ? 0 : 4); i++) {
        ...
        printf(" %sbat%du : %08X   %sbat%dl : %08X",
            is_601 ? "" : i < 4 ? "i" : "d",
            i & 3, ppc_state.spr[528+i*2],
            is_601 ? "" : i < 4 ? "i" : "d",
            i & 3, ppc_state.spr[529+i*2]
        );
        if (!is_601 || bat_entry->valid) {
            printf("   ;   range : %08X..%08X -> %08X..%08X   access : %s%s   protection : %s%s",
            ...
            printf("   ;   range : %8s..%8s -> %8s..%8s   access : %s%s   protection : %s%s",
                "", "", "", "", "__", "__", "_", "_"
            );
            printf("   (%s)", "invalid");
        }
...
void DppcDebugger::enter_debugger() {
    ...
    while (1) {
        ...
        if (cmd == "help") {
            ...
            static TimerInfo viaint_timer;
            TimerManager::get_instance()->add_oneshot_timer(viaint_timer, 0, [irq_bit](uint64_t, uint64_t){
