...
static uint32_t disasm_68k(uint32_t count, uint32_t address) {
    ...
    for (; power_on && count > 0; count--) {
        ...
        cout << setfill('0') << setw(8) << right << uppercase << hex << address;
        if (phys_addr != address) {
            cout << "->" << setfill('0') << setw(8) << right << uppercase << hex << phys_addr;
        }
        if (!name.empty()) {
        ...
        int i = 0;
        for (; i < code_size; i += 2)
            cout << setfill('0') << setw(4) << right << uppercase << hex << READ_WORD_BE_U(&code[i]) << " ";
        cout << setfill(' ') << setw((10 - (int)code_size) / 2 * 5) << "";
        cout << setfill(' ') << setw(10) << left << insn->mnemonic << insn->op_str << dec << endl;
    }
...
#define EMU_68K_START_PHYS       0xfff60000
#define EMU_68K_SIZE_PHYS           0xa0000
#define EMU_68K_START            0x68000000
#define EMU_68K_SIZE              0x2000000 // includes 0x69xxxxxx

#define EMU_68K_TABLE_START_PHYS 0xfff80000
#define EMU_68K_TABLE_START      0x68080000
...
void exec_single_68k()
{
    ...
    */
    ...
    uint32_t expected_instr_tab_entry = mmu_read_vmem<uint16_t>(cur_68k_pc) * 8 + emu_table_virt;
...
void dumpinstructionlog(uint64_t num) {
    ...
    do {
        ...
        if (irec->paddr != irec->addr) {
            cout << "->" << setfill('0') << setw(8) << right << uppercase << hex << irec->paddr;
        }
        if (!name.empty()) {
            cout << " " << setw(27) << left << setfill(' ') << name;
        }
