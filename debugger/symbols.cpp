    ...
    while (f >> hex >> start >> end >> type) {
        if (type == 0) {
            ...
            if (type == 1) {
                ...
                else if (binaries.back().segments.back().sections.empty() || start >= binaries.back().segments.back().sections.back().end ) {
                    if (previous_sym && (previous_sym->end == 0 || start < previous_sym->end))
                        previous_sym->end = start;
    ...
    if (_kmod) {
        try {
            ...
            while ((!(kmod & 3)) && kmod) {
                ...
                for (int i = 0; i < 8; i++) { val = mem_read_dbg(kmod + offsetof(kmod_info_t, name           ) + i * 8, 8); WRITE_QWORD_BE_A(&(((uint64_t*)(&info.name   ))[i]), val); if (!val) break; };
                for (int i = 0; i < 8; i++) { val = mem_read_dbg(kmod + offsetof(kmod_info_t, version        ) + i * 8, 8); WRITE_QWORD_BE_A(&(((uint64_t*)(&info.version))[i]), val); if (!val) break; };
        ...
        */
        ...
        do {
            ...
            for (int ncmd = 0; ncmd < hdr.ncmds; ncmd++, seg_addr += seg.cmdsize) {
                ...
                uint64_t val;
                for (int i = 0; i < 2; i++) { val = mem_read_dbg(seg_addr + offsetof(segment_command, segname) + i * 8, 8); WRITE_QWORD_BE_A(&(((uint64_t*)(&seg.segname))[i]), val); if (!val) break; };
                ...
                for (int nsect = 0; nsect < seg.nsects; nsect++, sec_addr += sizeof(sec)) {
                    ...
                    if (addr >= sec.addr + sec.size)
                        continue;
                    for (int i = 0; i < 2; i++) { val = mem_read_dbg(sec_addr + offsetof(section, sectname) + i * 8, 8); WRITE_QWORD_BE_A(&(((uint64_t*)(&sec.sectname))[i]), val); if (!val) break; };
                    for (int i = 0; i < 2; i++) { val = mem_read_dbg(sec_addr + offsetof(section, segname ) + i * 8, 8); WRITE_QWORD_BE_A(&(((uint64_t*)(&sec.segname ))[i]), val); if (!val) break; };

                    section_t *found_sec = nullptr;
                    if (found_seg) {
                        for (auto &loop_sec : found_seg->sections) {
                            if (loop_sec.end - loop_sec.start == sec.size && loop_sec.name == std::string(sec.segname) + ":" + sec.sectname) {
                                found_sec = &loop_sec;
                                break;
                            }
                        }
