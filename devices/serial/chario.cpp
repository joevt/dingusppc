    ...
    if (sel_rv > 0) {
        if (this->sockfd != -1) {
             if (FD_ISSET(this->sockfd, &readfds)) {
                ...
                if (received == -1) {
                    if (this->acceptfd == -1) {
                        //LOG_F(INFO, "socket \"%s\" sock read (not accepted yet) err: %s", this->path.c_str(), strerror(errno)); // this happens once before accept
                    }
                    else {
                        LOG_F(INFO, "socket \"%s\" sock read err: %s", this->path.c_str(), strerror(errno)); // should never happen
                    }
                }
                else if (received == 1) {
