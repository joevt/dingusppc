...
uint8_t EsccController::read(uint8_t reg_offset)
{
    ...
    default:
        value = 0;
        LOG_F(WARNING, "%s: read unimplemented register 0x%x",
            this->get_name_and_unit_address().c_str(), reg_offset);
    }
...
void EsccController::write(uint8_t reg_offset, uint8_t value)
{
    ...
    case LocalTalkReg::Start_B:
        ...
        break;
    default:
        LOG_F(WARNING, "%s: write unimplemented register 0x%x = 0x%02x",
            this->get_name_and_unit_address().c_str(), reg_offset, value);
    }
}
...
void EsccController::write_reg(int reg_num, uint8_t value)
{
    ...
    case WR2:
        ...
        this->int_vec = value;
        if (changed_bits || log_all_registers)
    ...
    case WR9:
        ...
        case WR9_CHANNEL_RESET_B: {
            VLOG_SCOPE_F(loguru::Verbosity_INFO, "%s:     Channel Reset B", this->get_name_and_unit_address().c_str());
        ...
        case WR9_CHANNEL_RESET_A: {
            VLOG_SCOPE_F(loguru::Verbosity_INFO, "%s:     Channel Reset A", this->get_name_and_unit_address().c_str());
...
void EsccChannel::write_reg(int reg_num, uint8_t value)
{
    ...
    case WR0:
        ...
        break;
    case WR1:
        if (changed_bits || this->controller->log_all_registers)
    ...
    case WR3:
        ...
        if (changed_bits & WR3_ENTER_HUNT_MODE) {
            if (!(this->read_regs[RR0] & RR0_SYNC_HUNT)) {
