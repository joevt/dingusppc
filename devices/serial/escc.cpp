...
EsccController::EsccController(bool for_pdm)
    : HWComponent(for_pdm ? "EsccPdm" : "Escc")
{
    // allocate channels
...
void EsccChannel::attach_backend(int id)
{
    ...
    case CHARIO_BE_SOCKET:
        this->chario = std::unique_ptr<CharIoBackEnd> (new CharIoSocket(this->get_name() + "_CharIoSocket",
            std::string("dingussocket") + ((this->unit_address & 255) ? "" : "b")));
        break;
#endif
    default:
...
static const DeviceDescription EsccPdm_Descriptor = {
    EsccController::create_pdm, {}, {}, HWCompType::MMIO_DEV
};

REGISTER_DEVICE(Escc, Escc_Descriptor);
REGISTER_DEVICE(EsccPdm, EsccPdm_Descriptor);
