...
void PsxCtrl::write(uint32_t /*rgn_start*/, uint32_t offset, uint32_t value, int size)
{
    ...
    case PsxReg::Page_Mappings_5:
        LOG_F(INFO, "PSX: write Page Mappings %d @%02x.%c = %0*x", (offset >> 3) - PsxReg::Page_Mappings_1 + 1, offset, SIZE_ARG(size), size * 2, value);
    ...
    for (int page = 0; page < 41; page++) {
        ...
        if ((ram_offset == -1) == (bank < 5 && bank_size_remaining[bank] > 0)) {
            if (ram_offset == -1) {
                ...
                AddressMapEntry *entry = add_ram_region(ram_offset, mem_size, &dram_ptr[mem_start]);
                mem_start += mem_size;
                ram_offset = -1;
                if (entry) {
