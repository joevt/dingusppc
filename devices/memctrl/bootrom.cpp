...
void FlashChip::set_controller(FlashController* controller)
{
            ...
            switch (value) {
                case 0x20:
                    if (this->supports_non_embedded) {
                        ...
                        for (int addr = 0; addr < 0x100000; addr++)
                            controller->rom_write(this, addr, 0xFF);
                        break;
                    }
                default:
...
uint32_t BootRomOW::read(uint32_t /*rgn_start*/, uint32_t offset, int size)
{
    ...
    value = 0;
    for (int i = 0; i < 4; i++) {
        uint8_t val = dynamic_cast<FlashChip *>(
...
{
    uint32_t index = chip->get_unit_address();
