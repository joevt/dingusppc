...
MPC106::MPC106(const std::string &dev_name)
...
void MPC106PCI::pci_cfg_write(uint32_t reg_offs, uint32_t value, const AccessDetails details) {
    if (reg_offs < 64) {
...
void MPC106PCI::setup_ram() {
    // 3 supports 1 GiB = real MPC106
    // 7 supports 2 GiB = fake MPC106; requires ROM patch to allow more than 1 GiB of RAM
...
static const DeviceDescription GrackleLombard_Descriptor = {
    MPC106::create, {}, GrackleLombard_Properties,
    HWCompType::MEM_CTRL | HWCompType::MMIO_DEV | HWCompType::PCI_HOST
};
