...
ATIRage::ATIRage(uint16_t dev_id) :
    PCIVideoCtrl(dev_id == ATI_RAGE_GT_DEV_ID ? "AtiRageGT" : "AtiRagePro"),
    ...
    if (unit_address == -1) {
        if (this->override_property(property, value)) {
            if (property == "gfxmem_size") {
                // allocate video RAM
                this->vram_size = this->get_property_int("gfxmem_size") << 20; // convert MBs to bytes
...
uint32_t ATIRage::read_reg(uint32_t reg_offset, uint32_t size) {
    ...
    if (reg_num != ATI_CRTC_INT_CNTL)
        LOG_F(ATIRAGE, "%s: read  %s %04x.%c = %0*x", this->get_name_and_unit_address().c_str(), get_reg_name(reg_num), reg_offset, SIZE_ARG(size), size * 2, (uint32_t)result);
...
void ATIRage::crtc_update() {
    ...
    this->draw_fb = true;
    LOG_F(INFO, "%s: clock_sel:%d fb_div:%02x", this->get_name_and_unit_address().c_str(), clock_sel, this->plls[VCLK0_FB_DIV + clock_sel]);
...
PostInitResultType ATIRage::device_postinit()
{
    ...
    this->vbl_cb = [this](uint8_t irq_line_state) {
        ...
        LOG_F(ATIINTERRUPT, "%s: irq_line_state:%d do_interrupt:%d CRTC_INT_CNTL:%08x", this->get_name_and_unit_address().c_str(), irq_line_state, do_interrupt, this->regs[ATI_CRTC_INT_CNTL]);
...
static const DeviceDescription AtiRageGT_Descriptor = {
    ATIRage::create_gt, {"Display@0"}, AtiRage_Properties, HWCompType::MMIO_DEV | HWCompType::PCI_DEV | HWCompType::VIDEO_CTRL
};

static const DeviceDescription AtiRagePro_Descriptor = {
    ATIRage::create_pro, {"Display@0"}, AtiRage_Properties, HWCompType::MMIO_DEV | HWCompType::PCI_DEV | HWCompType::VIDEO_CTRL
};
