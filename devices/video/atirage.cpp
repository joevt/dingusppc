...
uint32_t ATIRage::read_reg(uint32_t reg_offset, uint32_t size) {
    ...
    if (reg_num != ATI_CRTC_INT_CNTL)
        LOG_F(ATIRAGE, "%s: read  %s %04x.%c = %0*x", this->name.c_str(), get_reg_name(reg_num), reg_offset, SIZE_ARG(size), size * 2, (uint32_t)result);
...
PostInitResultType ATIRage::device_postinit()
{
    ...
    this->vbl_cb = [this](uint8_t irq_line_state) {
        ...
        LOG_F(ATIINTERRUPT, "%s: irq_line_state:%d do_interrupt:%d CRTC_INT_CNTL:%08x", this->name.c_str(), irq_line_state, do_interrupt, this->regs[ATI_CRTC_INT_CNTL]);
