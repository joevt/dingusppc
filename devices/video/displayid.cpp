        ...
        if (this->override_property(property, value)) {
            ...
            if (property == "mon_id") {
                ...
                if (is_apple_sense) {
                    ...
                    LOG_F(ERROR, "Ignored invalid Apple Sense");
                }
            }
...
{
    LOG_F(INFO, "dirs:%d levels:%d", dirs, levels);
    if (this->id_kind & Disp_Id_Kind::AppleSense) {
        switch ((dirs << 3) | levels) {
        case 0b0'100'011: LOG_F(INFO, "Sense line 2 pulled low; get sense line 1 and 0"); break;
        case 0b0'010'101: LOG_F(INFO, "Sense line 1 pulled low; get sense line 2 and 0"); break;
        case 0b0'001'110: LOG_F(INFO, "Sense line 0 pulled low; get sense line 2 and 1"); break;
        }
    }

    levels = apply_sense(levels, dirs);

    uint8_t i2cresult = 0b0'111;
    if (this->id_kind & Disp_Id_Kind::DDC2B) {
        ...
        i2cresult = update_ddc_i2c(sda, scl);
        levels &= 0b0'001 | i2cresult;
        LOG_F(INFO, "levels (i2c):%d", levels);
...
uint8_t DisplayID::apply_sense(uint8_t levels, uint8_t dirs)
{
    ...
    if (new_levels != levels) {
        levels = new_levels;
        LOG_F(INFO, "levels (dir):%d", levels);
    }
    ...
    for (int i = 1; i >= 0; i--) {
        for (int sense = 2; sense >= 0; sense--) { // loop sense line 2 to 0
            if ( ((1 << sense) & dirs) && !((1 << sense) & levels) ) { // if the sense line is driven low
                ...
                if (new_levels != levels) {
                    levels = new_levels;
                    LOG_F(INFO, "levels (ext):%d", levels);
                }
            }
        }
    }
...
uint8_t DisplayID::update_ddc_i2c(uint8_t sda, uint8_t scl)
{
    ...
    if (sda != this->last_sda) {
        ...
        if (this->last_scl) {
            if (!sda) {
                LOG_F(DDC, "DDC-I2C: START condition detected!");
    ...
    case I2CState::ACK:
        ...
        case I2CState::DEV_ADDR:
            LOG_F(WARNING, "DDC-I2C: ACK DEV_ADDR");
            if ((dev_addr & 0xFE) == 0xA0) {
                LOG_F(WARNING, "DDC-I2C: got address 0x%X", this->dev_addr);
                sda = 0; /* send ACK */
            } else {
        ...
        case I2CState::REG_ADDR:
            LOG_F(WARNING, "DDC-I2C: ACK REG_ADDR");
            this->next_state = I2CState::DATA;
            if (!this->reg_addr) {
                sda = 0; /* send ACK */
            } else {
            ...
            break;
        case I2CState::DATA:
            LOG_F(WARNING, "DDC-I2C: ACK DATA");
            this->next_state = I2CState::DATA;
            if (dev_addr & 1) {
                if (!sda) {
                    /* load next data byte */
                    if (data_pos < 128)
                        ...
                        this->byte = 0;
                    LOG_F(WARNING, "DDC-I2C: byte[%02X]:%02X", this->data_pos - 1, this->byte);
                } else {
            ...
            break;
        default:
            LOG_F(WARNING, "DDC-I2C: ACK STATE:%d", this->prev_state);
        }
        break;
    ...
    case I2CState::REG_ADDR:
        ...
        LOG_F(WARNING, "DDC-I2C: %s bit:%d byte:%02X",
    ...
    case I2CState::DATA:
        ...
        LOG_F(WARNING, "DDC-I2C: DATA bit:%d byte:%02X", this->bit_count, this->byte);
        if (this->bit_count++ >= 7) {
            this->bit_count  = 0;
            this->prev_state = this->next_state;
            this->next_state = I2CState::ACK;
        ...
        break;
    default:
        LOG_F(WARNING, "DDC-I2C: STATE:%d", this->next_state);
    }
