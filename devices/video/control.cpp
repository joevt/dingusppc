...
ControlVideo::ControlVideo()
    : PCIDevice("ControlVideo"), VideoCtrlBase(), HWComponent("ControlVideo")
{
...
uint32_t ControlVideo::read(uint32_t rgn_start, uint32_t offset, int size)
{
    ...
    if (rgn_start == this->regs_base) {
        ...
        case ControlRegs::HSERR:
            ...
            LOG_F(CONTROL, "%s: read  %s %03x.%c = %0*x", this->name.c_str(), get_name_controlreg(offset), offset, SIZE_ARG(size), size * 2, value);
            break;
        case ControlRegs::CNTTST:
            value = 0;
            LOG_F(ERROR, "%s: read  CNTTST %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            break;
        case ControlRegs::SWATCH_CTRL:
            value = this->swatch_ctrl;
            LOG_F(CONTROL, "%s: read  SWATCH_CTRL %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            break;
        case ControlRegs::GBASE:
            value = this->fb_base;
            LOG_F(CONTROL, "%s: read  GBASE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            break;
        case ControlRegs::ROW_WORDS:
            value = this->row_words;
            LOG_F(CONTROL, "%s: read  ROW_WORDS %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            break;
        ...
        case ControlRegs::MISC_ENABLES:
            value = this->enables;
            LOG_F(CONTROL, "%s: read  MISC_ENABLES %03x.%c", this->name.c_str(), offset, SIZE_ARG(size));
            break;
        case ControlRegs::GSC_DIVIDE:
            value = this->clock_divider;
            LOG_F(CONTROL, "%s: read  GSC_DIVIDE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            break;
        ...
        case ControlRegs::INT_STATUS:
            ...
            if (value != this->last_int_status) {
                LOG_F(CONTROL, "%s: read  (previous %d times) INT_STATUS %03x.%c = %0*x", this->name.c_str(), last_int_status_read_count, offset, SIZE_ARG(size), size * 2, value);
...
void ControlVideo::write(uint32_t rgn_start, uint32_t offset, uint32_t value, int size)
{
    ...
    if (rgn_start == this->regs_base) {
        ...
        case ControlRegs::CNTTST:
            ...
            if (value)
                LOG_F(ERROR, "%s: write CNTTST %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else
                LOG_F(CONTROL, "%s: write CNTTST %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            break;
        case ControlRegs::SWATCH_CTRL:
            if (value & ~0x7FF)
                LOG_F(ERROR, "%s: write SWATCH_CTRL %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else
                LOG_F(CONTROL, "%s: write SWATCH_CTRL %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            value &= 0x7FF;
        ...
        case ControlRegs::GBASE:
            if (value & ~0x3FFFE0)
                LOG_F(ERROR, "%s: write GBASE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else
                LOG_F(CONTROL, "%s: write GBASE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
        ...
        case ControlRegs::ROW_WORDS:
            if (value & ~0x7FE0)
                LOG_F(ERROR, "%s: write ROW_WORDS %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else
                LOG_F(CONTROL, "%s: write ROW_WORDS %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
        ...
        case ControlRegs::MISC_ENABLES:
            if (value & ~0xFFF)
                LOG_F(ERROR, "%s: write MISC_ENABLES %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else
                LOG_F(CONTROL, "%s: write MISC_ENABLES %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
        ...
        case ControlRegs::GSC_DIVIDE:
            if (value & ~3)
                LOG_F(ERROR, "%s: write GSC_DIVIDE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else
                LOG_F(CONTROL, "%s: write GSC_DIVIDE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
        ...
        case ControlRegs::REFRESH_COUNT:
            if (value & ~0x3FF)
                LOG_F(ERROR, "%s: write REFRESH_COUNT %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else
                LOG_F(CONTROL, "%s: write REFRESH_COUNT %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            break;
        case ControlRegs::INT_ENABLE:
            if (value & ~0xc)
                LOG_F(ERROR, "%s: write INT_ENABLE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            else {
                //LOG_F(CONTROL, "%s: write INT_ENABLE %03x.%c = %0*x", this->name.c_str(), offset, SIZE_ARG(size), size * 2, value);
            }
...
static std::unique_ptr<HWComponent> ControlDisplay_create() {
    return std::unique_ptr<DisplayID>(new DisplayID("ControlDisplay"));
}
