...
const regop regops[] = {
    #define oneregop(rw, aperture, reg, size, value) { rw, aperture, 0x ## reg, regsize_ ## size, flip_ ## size((uint32_t)0x ## value) },
    #include "nv47reg.h"
    #undef oneregop
};
    ...
    : PCIDevice("nv47"), VideoCtrlBase()
{
    ...
    /* 08 */ this->class_rev   = (0x030000 << 8) | 0xA1; // Display controller:VGA compatible controller:VGA controller; revision A1
...
//  /* 3C */ this->irq_line    = 0;
    ...
    if (this->attach_exp_rom_image(std::string("NV47.bin"))) {
...
void NV47::notify_bar_change(int bar_num)
{
    switch (bar_num) {
        case 0:
        case 1:
        case 3: change_one_bar(this->aperture_base[bar_num], this->aperture_size[bar_num], this->bars[bar_num] & ~15, bar_num); break;
        case 5: this->io_base = this->bars[bar_num] & ~3; break;
    }
}

uint32_t NV47::pci_cfg_read(uint32_t reg_offs, AccessDetails &details)
{
    if (reg_offs < 64) {
...
void NV47::pci_cfg_write(uint32_t reg_offs, uint32_t value, AccessDetails &details)
{
    if (reg_offs < 64) {
        if (reg_offs >= 4 && reg_offs < 8) {
...
static const DeviceDescription Nv47_Descriptor = {
    NV47::create, {}, Nv47_Properties, HWCompType::MMIO_DEV | HWCompType::PCI_DEV
};
