    ...
    if (unit_address == -1) {
        if (this->override_property(property, value)) {
            if (property == "gfxmem_size") {
                // allocate video RAM
                this->vram_size = get_property_int("gfxmem_size") << 20; // convert MBs to bytes
...
bool AtiMach64Gx::pci_io_read(uint32_t offset, uint32_t size, uint32_t* res)
{
    ...
    if ((offset >> 2) == ATI_CONFIG_CNTL) {
        ...
        LOG_F(ATIMACH64, "%s: read  %s %04x.%c = %0*x", this->name.c_str(), get_reg_name(offset >> 2), offset, SIZE_ARG(size), size * 2, (uint32_t)result);
    } else {
...
uint32_t AtiMach64Gx::read_reg(uint32_t reg_offset, uint32_t size)
{
    ...
    if (reg_num == ATI_CRTC_INT_CNTL) {
        ...
        if (int_cntl_val == last_int_cntl_val)
            ...
            if (int_cntl_count > 1)
                LOG_F(ATIINTERRUPT, "%s: read  %s %04x.%c = %0*x (%lld times)", this->name.c_str(), get_reg_name(reg_num), reg_offset, SIZE_ARG(size), size * 2, (uint32_t)last_int_cntl_val, int_cntl_count);
            LOG_F(ATIINTERRUPT, "%s: read  %s %04x.%c = %0*x", this->name.c_str(), get_reg_name(reg_num), reg_offset, SIZE_ARG(size), size * 2, (uint32_t)result);
            int_cntl_count = 1;
        ...
        last_int_cntl_val = -1;
        LOG_F(ATIMACH64, "%s: read  %s %04x.%c = %0*x", this->name.c_str(), get_reg_name(reg_num), reg_offset, SIZE_ARG(size), size * 2, (uint32_t)result);
    }
...
static const DeviceDescription AtiMach64Gx_Descriptor = {
    AtiMach64Gx::create, {"Display@0"}, AtiMach64gx_Properties, HWCompType::MMIO_DEV | HWCompType::PCI_DEV | HWCompType::VIDEO_CTRL
};
