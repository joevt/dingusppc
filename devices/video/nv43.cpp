    ...
    : PCIDevice("Nv43"), VideoCtrlBase(), HWComponent("Nv43")
{
    ...
    /* 06 */ this->status      = 0x02B0; // 0000  0 01 0  1 0 1 1  0 000 : Capabilities List, 66 MHz, Fast Back-to-Back, DevSel Speed: Medium
...
//  /* 3C */ this->irq_line    = 0;
    ...
    if (this->attach_exp_rom_image(std::string("NV43.bin"))) {
...
void NV43::notify_bar_change(int bar_num)
{
    switch (bar_num) {
        case 0:
        case 1:
        case 2: change_one_bar(this->aperture_base[bar_num], this->aperture_size[bar_num], this->bars[bar_num] & ~15, bar_num); break;
    }
}

uint32_t NV43::pci_cfg_read(uint32_t reg_offs, AccessDetails &details)
{
    if (reg_offs < 64) {
...
void NV43::pci_cfg_write(uint32_t reg_offs, uint32_t value, AccessDetails &details)
{
    if (reg_offs < 64) {
        if (reg_offs >= 4 && reg_offs < 8) {
