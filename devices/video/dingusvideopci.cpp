    ...
    : PCIVideoCtrl("DingusVideo"), HWComponent("DingusVideo")
{
    ...
    this->setup_bars({
        {0, 0xFFFFF000UL}, // base address for the HW registers (4KB)
        {1, 0xFFC00000UL}  // base address for the VRAM (4MB)
    });
    ...
    if (unit_address == -1) {
        if (this->override_property(property, value)) {
            if (property == "gfxmem_size") {
                ...
                this->bars_cfg[1] = -this->vram_size;
                return this;
            }
        }
    }
...
void DingusVideo::notify_bar_change(int bar_num) {
    switch (bar_num) {
    case 0: change_one_bar(this->regs_base,          0x1000, this->bars[bar_num] & ~15, bar_num); break;
    case 1: change_one_bar(this->vram_base, this->vram_size, this->bars[bar_num] & ~15, bar_num); break;
    }
}
...
static const PropMap DingusVideo_Properties = {
    {"gfxmem_size",
        new IntProperty(4, std::vector<uint32_t>({0, 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024}))},
};
