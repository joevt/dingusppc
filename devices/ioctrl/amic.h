...
public:
    ...
    virtual ~AmicSerialRcvDma() = default;
...
enum AMICReg : uint32_t {
    ...
    // VIA2 registers
    //                  = 0x26000, // byte, write 0x8f
    VIA2_Slot_IFR       = 0x26002,
    VIA2_IFR            = 0x26003,
    //                  = 0x26004, // byte, read
    //                  = 0x26010, // byte, write 0x40
...
public:
    AMIC(const std::string &dev_name);
    ~AMIC();

    static std::unique_ptr<HWComponent> create(const std::string &dev_name) {
        return std::unique_ptr<AMIC>(new AMIC(dev_name));
    }
