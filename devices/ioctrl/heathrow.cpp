...
HeathrowIC::HeathrowIC() : PCIDevice("mac-io_heathrow"), InterruptCtrl()
{
    ...
    this->device_id   = MIO_DEV_ID_HEATHROW;
...
uint32_t HeathrowIC::dma_read(uint32_t offset, int size) {
    ...
    default:
        LOG_F(WARNING, "Unsupported DMA channel %d  read @%02x.%c",
            offset >> 8, offset & 0xFF, SIZE_ARG(size));
    }
...
void HeathrowIC::dma_write(uint32_t offset, uint32_t value, int size) {
    ...
    case MIO_OHARE_DMA_AUDIO_OUT:
        ...
        break;
    default:
        LOG_F(WARNING, "Unsupported DMA channel %d write @%02x.%c = %0*x",
            offset >> 8, offset & 0xFF, SIZE_ARG(size), size * 2, value);
    }
}
