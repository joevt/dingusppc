...
IntSrc HeathrowIC::irq_id_to_src(uint64_t irq_id) {
    switch(irq_id) {
...
{
    VLOG_SCOPE_F(loguru::Verbosity_INTERRUPT, "%s: ack_int source:%s state:%d", this->name.c_str(), irq_id_to_name(irq_id), irq_line_state);
    ...
    LOG_F(INTERRUPT, "%s: native interrupt events:%08x.%08x levels:%08x.%08x change:%08x.%08x state:%d",
        this->name.c_str(), uint32_t(this->int_events >> 32), uint32_t(this->int_events), uint32_t(this->int_levels >> 32), uint32_t(this->int_levels), uint32_t(irq_id >> 32), uint32_t(irq_id), irq_line_state
    );
#endif
