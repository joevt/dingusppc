...
AMIC::AMIC(const std::string &dev_name)
    : MMIODevice(), HWComponent(dev_name)
{
...
PostInitResultType AMIC::device_postinit()
{
    ...
    TimerManager::get_instance()->add_cyclic_timer(pseudo_vbl_timer,
...
void AmicScsiDma::xfer_from_device() {
    ...
    do {
        ...
        int got_bytes = this->dev_obj->xfer_from(this, res.host_va, res.size);
        if (got_bytes > 0) {
...
void AmicScsiDma::xfer_to_device() {
    ...
    do {
        ...
        int got_bytes = this->dev_obj->xfer_to(this, res.host_va, res.size);
        if (got_bytes > 0) {
            this->addr_ptr += got_bytes;
        }
        if (got_bytes < res.size)
