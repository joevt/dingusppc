...
void AMIC::ack_int(uint64_t irq_id, uint8_t irq_line_state) {
    VLOG_SCOPE_F(loguru::Verbosity_5, "AMIC: ack_int source:%s state:%x", this->irq_src_to_name(this->irq_id_to_src(irq_id)), irq_line_state);
...
void AMIC::ack_slot_int(uint8_t slot_int, uint8_t irq_line_state) {
    VLOG_SCOPE_F(loguru::Verbosity_5, "AMIC: ack_slot_int source:%s state:%x", this->irq_src_to_name(this->irq_id_to_src(slot_int << SLOT_INT_SHIFT)), irq_line_state);
...
void AMIC::ack_via2_int(uint8_t via2_int, uint8_t irq_line_state) {
    VLOG_SCOPE_F(loguru::Verbosity_5, "AMIC: ack_via2_int source:%s state:%x", this->irq_src_to_name(this->irq_id_to_src(via2_int << VIA2_INT_SHIFT)), irq_line_state);
    if (irq_line_state) {
...
void AMIC::ack_cpu_int(uint8_t cpu_int, uint8_t irq_line_state) {
    VLOG_SCOPE_F(loguru::Verbosity_5, "AMIC: ack_cpu_int source:%s state:%x", this->irq_src_to_name(this->irq_id_to_src(cpu_int << CPU_INT_SHIFT)), irq_line_state);
    if (this->int_ctrl & CPU_INT_MODE) { // 68k interrupt emulation mode?
        ...
        if (!(this->int_ctrl & CPU_INT_FLAG)) {
            ...
            LOG_F(INFO, "AMIC: CPU INT already latched");
        }
...
void AMIC::ack_dma_int(uint64_t irq_id, uint8_t irq_line_state) {
    VLOG_SCOPE_F(loguru::Verbosity_5, "AMIC: ack_dma_int source:%s state:%x", this->irq_src_to_name(this->irq_id_to_src(irq_id)), irq_line_state);
...
{
    LOG_F(9, "AmicScsiDma::push_data len:%d", len);
    MapDmaResult res = mmu_map_dma_mem(this->addr_ptr, len, false);
    uint8_t *p_data = res.host_va;
    std::memcpy(p_data, src_ptr, len);
...
DmaPullResult AmicScsiDma::pull_data(uint32_t req_len, uint32_t *avail_len,
                                     uint8_t **p_data)
{
    LOG_F(9, "AmicScsiDma::pull_data len:%d", req_len);
