...
int32_t PCIBase::parse_self_unit_address_string(std::string unit_address) {
    std::regex unit_address_re("(1?[0-9A-F])(?:,([0-7]+))?", std::regex_constants::icase);
    std::smatch results;
    if (std::regex_match(unit_address, results, unit_address_re)) {
        int32_t result = int32_t(std::stol(results[1], nullptr, 16)) << 3;
        if (results[2].matched)
            result |= std::stol(results[2]);
        return result;
    } else {
        LOG_F(ERROR, "%s: Could not parse unit address \"%s\"", this->get_name().c_str(), unit_address.c_str());
        return -1;
    }
}
