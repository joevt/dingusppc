...
ActiontecBridge::ActiontecBridge(const std::string name) : PCIBridge(name), HWComponent(name)
{
    ...
    /* 06 */ this->status           = 0x0290; // 0000  0 01 0  1 0 0 1  0 000
                                              // : Capabilities list, Fast Back to Back, DevSel Speed: Medium
...
uint32_t ActiontecBridge::pci_cfg_read(uint32_t reg_offs, const AccessDetails details)
{
    if (reg_offs < 0x40) {
    ...
    switch (reg_offs) {
        ...
        case 0x84: return 0x00C00000;
            ...
            // +2: F602 = 1111 0 1 1 000 0 0 0 010
...
void ActiontecBridge::pci_cfg_write(uint32_t reg_offs, uint32_t value, const AccessDetails details)
{
    if (reg_offs < 0x40) {
