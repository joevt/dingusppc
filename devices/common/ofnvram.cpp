...
bool OfConfigAppl::set_var(std::string& var_name, std::string& value) {
    ...
    if (type == OF_VAR_TYPE_INT) {
        ...
        for (auto& var : of_vars) {
            auto i_type   = std::get<0>(var.second);
            auto i_offset = std::get<1>(var.second);
            if (i_type == OF_VAR_TYPE_STR) {
                uint16_t i_str_offset = READ_WORD_BE_A(&this->buf[i_offset]);
                if (i_str_offset < str_offset) {
                    WRITE_WORD_BE_A(&this->buf[i_offset], i_str_offset + str_len);
                }
            }
        }
        top = new_top;
