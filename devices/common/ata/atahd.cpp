...
void AtaHardDisk::insert_image(std::string filename) {
    ...
    if (sectors >= REAL_CHS_LIMIT) {
        LOG_F(WARNING, "%s: image file \"%s\" size (%f.3 GiB %f.3 GB) exceeds CHS limit (%f.3 GiB %f.3 GB)",
    ...
    if (sectors >= (1<<28)) {
        ...
        LOG_F(WARNING, "%s: image file \"%s\" size (%f.3 GiB %f.3 GB) exceeds LBA28 limit (%f.3 GiB %f.3 GB)",
            this->get_name_and_unit_address().c_str(),
            filename.c_str(),
            1.0 * ATA_HD_SEC_SIZE * this->total_sectors / (1<<30),
            1.0 * ATA_HD_SEC_SIZE * this->total_sectors / (1000000000),
    ...
    if (sectors >= (1LL<<32)) {
        LOG_F(WARNING, "%s: image file \"%s\" size (%f.3 GiB %f.3 GB) exceeds LBA32 limit (%f.3 GiB %f.3 GB)",
            this->get_name_and_unit_address().c_str(),
            filename.c_str(),
            1.0 * ATA_HD_SEC_SIZE * this->total_sectors / (1<<30),
            1.0 * ATA_HD_SEC_SIZE * this->total_sectors / (1000000000),
    ...
    if (sectors >= (1LL<<48)) {
        ABORT_F("%s: image file \"%s\" size (%f.3 GiB %f.3 GB) exceeds LBA48 limit (%f.3 GiB %f.3 GB)",
            this->get_name_and_unit_address().c_str(),
            filename.c_str(),
            1.0 * ATA_HD_SEC_SIZE * this->total_sectors / (1<<30),
            1.0 * ATA_HD_SEC_SIZE * this->total_sectors / (1000000000),
