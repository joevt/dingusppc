...
void AtaHardDisk::prepare_identify_info() {
    ...
    // report LBA capacity
    WRITE_DWORD_LE_A(&buf_ptr[ident_capacity], (uint32_t)std::min(this->total_sectors, (uint64_t)(1ULL<<28) - (uint64_t)1));
    ...
    if (this->total_sectors >= (1<<28)) {
        ...
        WRITE_QWORD_LE_A(&buf_ptr[ident_48bitlba], std::min((uint64_t)this->total_sectors, (uint64_t)(1ULL<<48) - (uint64_t)1));
    }
}
