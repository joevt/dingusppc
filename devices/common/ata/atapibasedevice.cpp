...
uint16_t AtapiBaseDevice::read(const uint8_t reg_addr) {
    ...
    case ATA_Reg::DATA:
        if (has_data()) {
            ...
            this->xfer_cnt -= 2;
            if (this->xfer_cnt <= 0) {
                //LOG_F(INFO, "%s: Read completed (%d, %d, %d)", this->get_name_and_unit_address().c_str(), this->xfer_cnt, r_int_reason & ATAPI_Int_Reason::IO, r_int_reason & ATAPI_Int_Reason::CoD);
                this->r_status &= ~DRQ;
...
int AtapiBaseDevice::perform_command() {
    VLOG_SCOPE_F(this->r_command != ATAPI_PACKET ? loguru::Verbosity_WARNING : 10, "%s: perform_command 0x%X", this->get_name_and_unit_address().c_str(), this->r_command);
