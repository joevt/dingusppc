...
HWComponent* IdeChannel::set_property(const std::string &value, int32_t unit_address) {
    ...
    if (unit_address == -1) {
        ...
        for (ata_id = 0; ata_id < 2; ata_id++) {
            ...
            if (ata_device) {
                result = ata_device->set_property(std::is_same<T,AtaHardDisk>::value ? "hdd_img" : "cdr_img", value, unit_address);
                if (result)
                    return result;
            }
        }
        ...
        // look for unused ID
        for (ata_id = 0; ata_id < 2 &&
             this->devices[ata_id]; ata_id++) {}
        if (ata_id == 2)
            return nullptr;
    }
    else {
    ...
    if (this->devices[ata_id])
        ata_device = dynamic_cast<T *>(this->devices[ata_id]);
    else
        ata_device = dynamic_cast<T *>(MachineFactory::create_device(this, std::is_same<T,AtaHardDisk>::value ? "AtaHardDisk@" : "AtapiCdrom@" + std::to_string(ata_id)));
    if (ata_device)
...
void IdeChannel::write(const uint8_t reg_addr, const uint32_t val, const int size)
{
    ...
    if (reg_addr == DEVICE_HEAD) {
        ...
        LOG_F(IDE_CHANNEL, "%s: cur_dev = %d (%s)", this->name.c_str(), this->cur_dev, base ? base->get_name().c_str() : "AtiNullDevice");
    }
