...
HWComponent* ScsiBus::set_property(const std::string &value, int32_t unit_address) {
    ...
    if (unit_address == -1) {
        ...
        for (scsi_id = 0; scsi_id < SCSI_MAX_DEVS; scsi_id++) {
            ...
            if (scsi_device) {
                result = scsi_device->set_property(std::is_same<T,ScsiHardDisk>::value ? "hdd_img" : "cdr_img", value, unit_address);
                if (result)
                    return result;
            }
        }
    ...
    if (this->devices[scsi_id])
        ...
        scsi_device = dynamic_cast<T *>(MachineFactory::create_device(this, (std::is_same<T,ScsiHardDisk>::value ? "ScsiHardDisk@" : "ScsiCdrom@") + std::to_string(scsi_id)));
    if (scsi_device)
...
int ScsiBus::switch_phase(int id, int new_phase)
{
    ...
    LOG_F(SCSIBUS, "%s: changing bus phase from %s to %s", name.c_str(), get_name_bus_phase(old_phase), get_name_bus_phase(new_phase));
