...
static bool debug_scsi_log = false;

#define SCSI_LOG_IF_F(verbosity_name, ...) \
    do { if (debug_scsi_log) { VLOG_F(loguru::Verbosity_ ## verbosity_name, __VA_ARGS__); last_log_message = LastLog::Misc; } } while (0)
...
uint16_t Sc53C94::pseudo_dma_read()
{
    ...
    if (this->data_fifo_pos >= 2) {
        ...
        SCSI_LOG_IF_F(CURIO, "fifo_pos:%d->%d in %s (popped data:%04x)", this->data_fifo_pos, this->data_fifo_pos - 2, __func__, data_word);
...
void Sc53C94::exec_command()
{
    ...
    case CMD_XFER:
        if (!this->is_initiator) {
            ...
            SCSI_LOG_F(CURIO, "%s: state changed to %s in %s CMD_XFER", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
            this->sequencer();
        }
        break;
    case CMD_COMPLETE_STEPS:
        ...
        SCSI_LOG_F(CURIO, "%s: state changed to %s in %s CMD_COMPLETE_STEPS", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
        this->sequencer();
        break;
    case CMD_MSG_ACCEPTED:
        ...
        SCSI_LOG_IF_F(CURIO, "int_status = INTSTAT_SR | INTSTAT_DIS = %02x in %s CMD_MSG_ACCEPTED", this->int_status, __func__);
        this->update_irq();
        exec_next_command();
        break;
        /*
    ...
    case CMD_SELECT_NO_ATN:
        ...
        this->cur_state = SeqState::BUS_FREE;
        SCSI_LOG_F(CURIO, "%s: state changed to %s in %s CMD_SELECT_NO_ATN", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
        this->sequencer();
    ...
    case CMD_SELECT_WITH_ATN:
        ...
        this->cur_state = SeqState::BUS_FREE;
        SCSI_LOG_F(CURIO, "%s: state changed to %s in %s CMD_SELECT_WITH_ATN", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
        this->sequencer();
        SCSI_LOG_F(CURIO, "%s: SELECT WITH ATN command started", this->name.c_str());
        break;
...
void Sc53C94::fifo_push(const uint8_t data)
{
    if (this->data_fifo_pos < DATA_FIFO_MAX) {
        ...
        SCSI_LOG_IF_F(CURIO, "fifo_pos:%d->%d in %s (pushed data:%02x)", this->data_fifo_pos - 1, this->data_fifo_pos, __func__, data);
    } else {
...
uint8_t Sc53C94::fifo_pop()
{
    ...
    if (this->data_fifo_pos < 1) {
        ...
        SCSI_LOG_IF_F(CURIO, "fifo_pos:%d->%d in %s (popped data:%02x)", this->data_fifo_pos, this->data_fifo_pos - 1, __func__, data);
        this->data_fifo_pos--;
...
void Sc53C94::seq_defer_state(uint64_t delay_ns)
{
    ...
    seq_timer_id = TimerManager::get_instance()->add_oneshot_timer(
        ...
        [this]() {
            ...
            SCSI_LOG_F(CURIO, "%s: state changed to %s in %s seq_defer_state timer", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
            this->sequencer();
    });
}

extern string hex_string(const uint8_t *p, int len);
...
void Sc53C94::sequencer()
{
    ...
    case SeqState::SEL_END:
        if (this->bus_obj->end_selection(this->my_bus_id, this->target_id)) {
            ...
            SCSI_LOG_IF_F(CURIO, "int_status = %02x (from %x) = %02x in %s SEL_END", this->cmd_steps->status, this->cmd_steps->seq_id, this->int_status, __func__);
            this->bus_obj->disconnect(this->my_bus_id);
            this->cur_state = SeqState::IDLE;
            SCSI_LOG_F(CURIO, "%s: state changed to %s in %s SEL_END", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
            this->update_irq();
            exec_next_command();
        }
        break;
    ...
    case SeqState::CMD_COMPLETE:
        this->seq_step   = this->cmd_steps->step_num;
        this->int_status = this->cmd_steps->status;
        SCSI_LOG_IF_F(CURIO, "int_status = %02x = %02x (from %x) in %s CMD_COMPLETE", this->cmd_steps->status, this->cmd_steps->seq_id, this->int_status, __func__);
        this->cur_state = SeqState::IDLE;
        SCSI_LOG_F(CURIO, "%s: state changed to %s in %s CMD_COMPLETE", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
        this->update_irq();
        exec_next_command();
        break;
    case SeqState::XFER_BEGIN:
        ...
        case ScsiPhase::DATA_OUT:
            ...
            if (this->is_dma_cmd) {
                ...
                SCSI_LOG_F(CURIO, "%s: state changed to %s in %s XFER_BEGIN", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
                break;
            }
            this->bus_obj->push_data(this->target_id, this->data_fifo, this->data_fifo_pos);
            SCSI_LOG_IF_F(CURIO, "fifo_pos:%d->%d in %s XFER_BEGIN.1 (popped data:%s)", this->data_fifo_pos, 0, __func__, hex_string(this->data_fifo, this->data_fifo_pos).c_str());
            this->data_fifo_pos = 0;
            this->cur_state = SeqState::XFER_END;
            SCSI_LOG_F(CURIO, "%s: state changed to %s in %s XFER_BEGIN.2", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
            this->sequencer();
            break;
        case ScsiPhase::DATA_IN:
            ...
            SCSI_LOG_F(CURIO, "%s: state changed to %s in %s DATA_IN.1", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
            this->rcv_data();
            if (!(this->is_dma_cmd)) {
                this->cur_state = SeqState::XFER_END;
                SCSI_LOG_F(CURIO, "%s: state changed to %s in %s DATA_IN.2", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
                this->sequencer();
            }
        }
        break;
    case SeqState::XFER_END:
        ...
        this->cur_state = SeqState::IDLE;
        SCSI_LOG_F(CURIO, "%s: state changed to %s in %s XFER_END", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
        this->update_irq();
        exec_next_command();
        break;
    ...
    case SeqState::RCV_MESSAGE:
        ...
        if (this->is_initiator) {
            if (this->cur_state == SeqState::RCV_STATUS) {
                ...
                this->cmd_steps++;
                this->cur_state = this->cmd_steps->next_step;
                SCSI_LOG_F(CURIO, "%s: state changed to %s (from %x) in %s RCV_MESSAGE", this->name.c_str(), get_name_sequence(this->cur_state), this->cmd_steps->seq_id, __func__);
                this->sequencer();
            }
        }
        break;
...
void Sc53C94::notify(ScsiMsg msg_type, int param)
{
    ...
    case ScsiMsg::CONFIRM_SEL:
        ...
        if (this->target_id == param) {
            ...
            SCSI_LOG_F(CURIO, "%s: state changed to %s in %s CONFIRM_SEL", this->name.c_str(), get_name_sequence(this->cur_state), __func__);
            this->sequencer();
        } else {
            SCSI_LOG_F(WARNING, "%s: ignore invalid selection confirmation message",
                  this->name.c_str());
        }
        break;
    case ScsiMsg::BUS_PHASE_CHANGE:
        ...
        if (param != ScsiPhase::BUS_FREE && this->cmd_steps != nullptr) {
            this->cmd_steps++;
            this->cur_state = this->cmd_steps->next_step;
            SCSI_LOG_F(CURIO, "%s: state changed to %s (from %x) in %s BUS_PHASE_CHANGE", this->name.c_str(), get_name_sequence(this->cur_state), this->cmd_steps->seq_id, __func__);
            this->sequencer();
        }
        break;
    default:
...
int Sc53C94::send_data(uint8_t* dst_ptr, int count)
{
    ...
    SCSI_LOG_IF_F(CURIO, "fifo_pos:%d->%d in %s (popped data:%s)", this->data_fifo_pos, this->data_fifo_pos - actual_count, __func__, hex_string(this->data_fifo, actual_count).c_str());
    ...
    if (this->data_fifo_pos > 0) {
        ...
        this->cmd_steps++;
        this->cur_state = this->cmd_steps->next_step;
        SCSI_LOG_F(CURIO, "%s: state changed to %s (from %x) in %s", this->name.c_str(), get_name_sequence(this->cur_state), this->cmd_steps->seq_id, __func__);
        this->sequencer();
    }
...
bool Sc53C94::rcv_data()
{
    ...
    SCSI_LOG_IF_F(CURIO, "target_id:%d req_count:%d fifo_pos:%d->%d in %s (pushed data: %s)", this->target_id, req_count,
        this->data_fifo_pos, this->data_fifo_pos + req_count, __func__, hex_string(&this->data_fifo[data_fifo_pos], req_count).c_str()
    );
...
void Sc53C94::real_dma_xfer_out()
{
    ...
    if (this->xfer_count) {
        if (this->data_fifo_pos) {
            SCSI_LOG_F(ERROR, "xfer_out_iteration:%d xfer_count:%d fifo_pos:%d", xfer_out_iteration, this->xfer_count, this->data_fifo_pos);
        }
        else {
            SCSI_LOG_F(CURIO, "xfer_out_iteration:%d xfer_count:%d fifo_pos:%d", xfer_out_iteration, this->xfer_count, this->data_fifo_pos);
        }
        uint32_t got_bytes;
        uint8_t* src_ptr;
        ...
        SCSI_LOG_IF_F(CURIO, "fifo_pos:%d->%d in %s (pushed data:%s)", this->data_fifo_pos, got_bytes, __func__, hex_string(src_ptr, got_bytes).c_str());
        this->data_fifo_pos = got_bytes;
        this->bus_obj->push_data(this->target_id, this->data_fifo, this->data_fifo_pos);

        this->xfer_count -= this->data_fifo_pos;
        SCSI_LOG_IF_F(CURIO, "fifo_pos:%d->%d in %s (popped data:%s)", this->data_fifo_pos, 0, __func__, hex_string(this->data_fifo, this->data_fifo_pos).c_str());
        this->data_fifo_pos = 0;
        if (!this->xfer_count) {
            this->status |= STAT_TC; // signal zero transfer count
            SCSI_LOG_IF_F(CURIO, "status |= STAT_TC = %02x in %s", this->status, __func__);
...
void Sc53C94::real_dma_xfer_in()
{
    ...
    if (xfer_in_iteration == 1 || this->xfer_count < 100) {
        SCSI_LOG_F(CURIO, "xfer_in_iteration:%d xfer_count:%d fifo_pos:%d", xfer_in_iteration, this->xfer_count, this->data_fifo_pos);
    }
...
void Sc53C94::dma_wait() {
    if (this->cur_bus_phase == ScsiPhase::DATA_IN && this->cur_state == SeqState::RCV_DATA) {
        ...
        SCSI_LOG_F(CURIO, "%s: dma_wait sequence:%s phase:%s", this->name.c_str(), get_name_sequence(this->cur_state), get_name_phase(this->cur_bus_phase));
...
{
    SCSI_LOG_F(CURIO, "[ %s: dma_start phase:%s", this->name.c_str(), get_name_phase(this->cur_bus_phase));
    dma_wait();
    SCSI_LOG_F(CURIO, "] %s: dma_start", this->name.c_str());
}
