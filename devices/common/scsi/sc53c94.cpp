...
void Sc53C94::notify(ScsiMsg msg_type, int param)
{
    ...
    case ScsiMsg::BUS_PHASE_CHANGE:
        ...
        if (param != ScsiPhase::BUS_FREE && this->cmd_steps != nullptr) {
            this->cmd_steps++;
            this->cur_state = this->cmd_steps->next_step;
            SCSI_LOG_F(CURIO, "%s: state changed to %s (from %x) in %s BUS_PHASE_CHANGE",
                this->name.c_str(), get_name_sequence(this->cur_state), this->cmd_steps->seq_id, __func__);
            this->sequencer();
        }
        break;
...
int Sc53C94::send_data(uint8_t* dst_ptr, int count)
{
    ...
    if (this->data_fifo_pos > 0) {
        ...
        this->cmd_steps++;
        this->cur_state = this->cmd_steps->next_step;
        SCSI_LOG_F(CURIO, "%s: state changed to %s (from %x) in %s", this->name.c_str(),
            get_name_sequence(this->cur_state), this->cmd_steps->seq_id, __func__);
        this->sequencer();
    }
