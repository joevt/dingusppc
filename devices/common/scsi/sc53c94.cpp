...
void Sc53C94::exec_command()
{
    ...
    case CMD_SET_ATN:
        ...
        SCSI_LOG_F(CURIO, "%s: state changed to %s in %s CMD_SET_ATN",
            this->name.c_str(), get_name_sequence(this->cur_state), __func__);
        this->sequencer();
...
void Sc53C94::sequencer()
{
    ...
    case SeqState::XFER_BEGIN:
        ...
        case ScsiPhase::DATA_IN:
            ...
            SCSI_LOG_F(CURIO, "%s: state changed to %s in %s DATA_IN.1",
                this->name.c_str(), get_name_sequence(this->cur_state), __func__);
            this->rcv_data();
            if (!(this->is_dma_cmd)) {
                this->cur_state = SeqState::XFER_END;
                SCSI_LOG_F(CURIO, "%s: state changed to %s in %s DATA_IN.2",
                    this->name.c_str(), get_name_sequence(this->cur_state), __func__);
                this->sequencer();
            }
        }
    ...
    case SeqState::RCV_MESSAGE:
        ...
        if (this->is_initiator) {
            if (this->cur_state == SeqState::RCV_STATUS) {
                ...
                this->cmd_steps++;
                this->cur_state = this->cmd_steps->next_step;
                SCSI_LOG_F(CURIO, "%s: state changed to %s (from %x) in %s RCV_MESSAGE",
                    this->name.c_str(), get_name_sequence(this->cur_state), this->cmd_steps->seq_id, __func__);
                this->sequencer();
            }
        }
        break;
...
void Sc53C94Dev::notify(ScsiMsg msg_type, int param)
{
    ...
    case ScsiMsg::BUS_PHASE_CHANGE:
        ...
        if (param != ScsiPhase::BUS_FREE && this->ctrl_obj->cmd_steps != nullptr) {
            ...
            SCSIDEV_LOG_F(CURIO, "%s: state changed to %s (from %x) in %s BUS_PHASE_CHANGE",
                this->name.c_str(), get_name_sequence(this->ctrl_obj->cur_state), this->ctrl_obj->cmd_steps->seq_id, __func__);
            this->ctrl_obj->sequencer();
        }
        break;
    default:
