...
Sc53C825PCI::Sc53C825PCI(const std::string &dev_name)
    : PCIDevice(dev_name), HWComponent(dev_name)
{
    ...
    /* 06 */ this->status         = 0x0230; // 4:Capabilities, 5:66 MHz, 7:Fast back to back, 9:DEVSEL medium
...
void Sc53C825PCI::change_one_bar(uint32_t &aperture, uint32_t aperture_size, uint32_t aperture_new, int bar_num) {
...
void Sc53C825PCI::notify_bar_change(int bar_num)
{
    switch (bar_num) {
...
uint32_t Sc53C825PCI::pci_cfg_read(uint32_t reg_offs, const AccessDetails details)
{
    if (reg_offs < 64) {
...
void Sc53C825PCI::pci_cfg_write(uint32_t reg_offs, uint32_t value, const AccessDetails details)
{
    if (reg_offs < 64) {
...
int Sc53C825PCI::io_access_allowed(uint32_t offset) {
...
bool Sc53C825PCI::pci_io_read(uint32_t offset, uint32_t size, uint32_t* res) {
...
bool Sc53C825PCI::pci_io_write(uint32_t offset, uint32_t value, uint32_t size) {
    int bar = this->io_access_allowed(offset);
    if (bar < 0) {
        return false;
    }
...
uint32_t Sc53C825PCI::read(uint32_t rgn_start, uint32_t offset, int size)
{
...
void Sc53C825PCI::write(uint32_t rgn_start, uint32_t offset, uint32_t value, int size)
{
    if (rgn_start == this->aperture_base[5] && offset < aperture_size[5]) {
        LOG_F(
...
Sc53C825Dev::Sc53C825Dev(const std::string &dev_name)
    : ScsiDevice(dev_name), HWComponent(dev_name)
{
...
PostInitResultType Sc53C825::device_postinit()
{
    ...
    this->dev_obj = dynamic_cast<ScsiDevice*>(gMachineObj->get_comp_by_name("Sc53C825Dev"));
...
static const DeviceDescription Sc53C825_Descriptor = {
    Sc53C825::create, {"Scsi53C825"}, {}, HWCompType::SCSI_HOST
};

REGISTER_DEVICE(Sc53C825, Sc53C825_Descriptor);

static const DeviceDescription Sc53C825PCI_Descriptor = {
    Sc53C825PCI::create, {"Scsi53C825@0"}, {}, HWCompType::MMIO_DEV | HWCompType::PCI_DEV
};
