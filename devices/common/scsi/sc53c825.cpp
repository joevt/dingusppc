...
void Sc53C825::reset_device()
{
    ...
    this->scsi_status &= STAT_PHASE_MASK; // reset doesn't affect bus phase bits
    SCSI_LOG_IF_F(CURIO, "status:%02x in %s", this->scsi_status, __func__);
...
uint8_t Sc53C825::read(uint8_t reg_offset)
{
    ...
    case Read::Reg53C825::Status: {
        ...
        value = (this->scsi_status & 0xF8) | bus_phase;
        break;
    }
    case Read::Reg53C825::Int_Status:
        value = this->int_status;
        if (this->irq) {
            this->scsi_status &= ~(STAT_GE | STAT_PE | STAT_GCV);
            SCSI_LOG_IF_F(CURIO, "status &= ~(STAT_GE | STAT_PE | STAT_GCV) = %02x in %s", this->scsi_status, __func__);
            this->int_status = 0;
...
uint16_t Sc53C825::pseudo_dma_read()
{
    ...
    if (this->data_fifo_pos >= 2) {
        ...
        if (this->is_dma_cmd) {
            ...
            if (!this->xfer_count) {
                is_done = true;
                this->scsi_status |= STAT_TC; // signal zero transfer count
                SCSI_LOG_IF_F(CURIO, "status |= STAT_TC = %02x in %s", this->scsi_status, __func__);
...
void Sc53C825::update_command_reg(uint8_t cmd)
{
    ...
    if (this->cmd_fifo_pos < 2) {
        ...
        this->scsi_status |= STAT_GE; // signal IOE/Gross Error
        SCSI_LOG_IF_F(CURIO, "status |= STAT_GE = %02x in %s", this->scsi_status, __func__);
    }
}
...
void Sc53C825::update_irq()
{
    ...
    if (new_irq != this->irq) {
        this->irq = new_irq;
        this->scsi_status = (this->scsi_status & ~STAT_INT) | (new_irq << 7);
        SCSI_LOG_IF_F(CURIO, "status |= STAT_INT(%d) = %02x in %s", new_irq, this->scsi_status, __func__);
        this->int_ctrl->ack_int(this->irq_id, new_irq);
    } else {
        SCSI_LOG_IF_F(CURIO, "STAT_INT(%d) unchanged so status(%02x) is unchanged in %s", new_irq, this->scsi_status, __func__);
    }
}

void Sc53C825::update_phase_latch() {
    if (this->config2 & CFG2_ENF) {
        if (this->phase_latch_closed) {
        ...
        if (this->ctrl_obj->cmd_steps != nullptr) {
            if (this->ctrl_obj->cur_bus_phase == this->ctrl_obj->cmd_steps->expected_phase) {
                ...
                if (this->ctrl_obj->cmd_steps->next_state == SeqState::CMD_COMPLETE) {
                    this->ctrl_obj->update_phase_latch();
...
int Sc53C825::xfer_from(DmaChannel *ch_obj, uint8_t *buf, int len) {
    int bytes_moved = 0;
    ...
    if (this->data_fifo_pos) {
        ...
        if (!this->xfer_count) {
            this->scsi_status |= STAT_TC; // signal zero transfer count
            SCSI_LOG_IF_F(CURIO, "status |= STAT_TC = %02x in %s.1", this->scsi_status, __func__);
            this->cur_state = SeqState::XFER_END;
    ...
    if (this->bus_obj->pull_data(this->target_id, buf, len)) {
        ...
        if (!this->xfer_count) {
            this->scsi_status |= STAT_TC; // signal zero transfer count
            SCSI_LOG_IF_F(CURIO, "status |= STAT_TC = %02x in %s.2", this->scsi_status, __func__);
            this->cur_state = SeqState::XFER_END;
...
int Sc53C825::xfer_to(DmaChannel *ch_obj, uint8_t *buf, int len) {
    int bytes_moved = 0;
...
static const DeviceDescription Scsi53C825_Descriptor = {
    ScsiBus::create, {"Sc53C825Dev@7"}, {}, HWCompType::SCSI_BUS
};
