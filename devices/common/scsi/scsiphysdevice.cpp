...
void ScsiPhysDevice::notify(ScsiNotification notif_type, int param)
{
    if (notif_type == ScsiNotification::BUS_PHASE_CHANGE) {
        ...
        case ScsiPhase::RESET:
            LOG_F(SCSIDEVICE, "%s: bus reset acknowledged", this->get_name_and_unit_address().c_str());
            break;
        case ScsiPhase::SELECTION:
            ...
            if (this->bus_obj->get_data_lines() & (1 << scsi_id)) {
                ...
                if (this->bus_settle_timer.active)
                    LOG_F(ERROR, "%s: bus_settle_timer already active", this->get_name_and_unit_address().c_str());
    ...
    while (remainder) {
        if (this->data_size) {
            ...
            if (chunk_size) {
                ...
                LOG_F(SCSIDEVICE, "%s: send_data data_size:%d -> %d", this->get_name_and_unit_address().c_str(),
...
int ScsiPhysDevice::rcv_data(const uint8_t* src_ptr, const int count)
{
    ...
    if (this->cur_phase == ScsiPhase::DATA_OUT) {
        ...
        LOG_F(SCSIDEVICE, "%s: rcv_data data_size:%d -> %d", this->get_name_and_unit_address().c_str(),
...
int32_t ScsiPhysDevice::parse_self_unit_address_string(const std::string unit_address_string) {
    return ScsiPhysDevice::parse_unit_address_string(unit_address_string);
}

int32_t ScsiPhysDevice::parse_unit_address_string(const std::string unit_address_string) {
