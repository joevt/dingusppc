...
void ScsiDevice::notify(ScsiMsg msg_type, int param)
{
    if (msg_type == ScsiMsg::BUS_PHASE_CHANGE) {
        ...
        LOG_F(SCSIDEVICE, "%s: unhandled notification type %d", this->get_name().c_str(), msg_type);
    }
}
...
void ScsiDevice::next_step()
{
    ...
    case ScsiPhase::DATA_OUT:
        LOG_F(SCSIDEVICE, "%s: DATA_OUT data_size:%d incoming_size:%d in %s", this->get_name().c_str(), this->data_size, this->incoming_size, __func__);
