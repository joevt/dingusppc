...
void ScsiDevice::notify(ScsiMsg msg_type, int param)
{
    if (msg_type == ScsiMsg::BUS_PHASE_CHANGE) {
        ...
        case ScsiPhase::SELECTION:
            LOG_F(SCSIDEVICE, "%s: checking selected for %d", this->get_name_and_unit_address().c_str(), scsi_id);
            ...
            break;
        default:
            LOG_F(SCSIDEVICE, "%s: ScsiMsg::BUS_PHASE_CHANGE unhandled phase %d", this->get_name_and_unit_address().c_str(), param);
        }
    }
    else {
        LOG_F(SCSIDEVICE, "%s: unhandled notification type %d", this->get_name_and_unit_address().c_str(), msg_type);
    }
}
...
void ScsiDevice::next_step()
{
    ...
    case ScsiPhase::DATA_OUT:
        LOG_F(SCSIDEVICE, "%s: DATA_OUT data_size:%d incoming_size:%d in %s", this->get_name_and_unit_address().c_str(), this->data_size, this->incoming_size, __func__);
...
void ScsiDevice::prepare_xfer(ScsiBus* bus_obj, int& bytes_in, int& bytes_out)
{
    ...
    case ScsiPhase::MESSAGE_IN:
        ...
        break;
    default:
        ABORT_F("%s: unhandled phase %d in prepare_xfer()", this->get_name_and_unit_address().c_str(), this->cur_phase);
    }
}
...
int ScsiDevice::send_data(uint8_t* dst_ptr, const int count)
{
    ...
    LOG_F(SCSIDEVICE, "%s: send_data data_size:%d -> %d", this->get_name_and_unit_address().c_str(), this->data_size, this->data_size - count);
...
int ScsiDevice::rcv_data(const uint8_t* src_ptr, const int count)
{
    ...
    LOG_F(SCSIDEVICE, "%s: rcv_data data_size:%d -> %d", this->get_name_and_unit_address().c_str(), this->data_size, this->data_size + count);
    this->data_ptr  += count;
...
bool ScsiDevice::check_lun() {
    if (this->cmd_buf[1] >> 5 != this->lun) {
        LOG_F(ERROR, "%s: non-matching LUN", this->get_name_and_unit_address().c_str());
...
void ScsiDevice::illegal_command(const uint8_t *cmd) {
    LOG_F(ERROR, "%s: unsupported command: 0x%02x", this->get_name_and_unit_address().c_str(), cmd[0]);
    this->status = ScsiStatus::CHECK_CONDITION;
    this->sense  = ScsiSense::ILLEGAL_REQ;
