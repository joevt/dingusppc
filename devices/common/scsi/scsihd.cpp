...
void ScsiHardDisk::process_command() {
    ...
    LOG_F(SCSIHD, "%s: process_command 0x%X", this->name.c_str(), cmd_buf[0]);
...
/*
    case ScsiCommand::READ_LONG_10:
        lba = READ_DWORD_BE_U(&cmd[2]);
        this->read_long_10(lba, READ_WORD_BE_U(&cmd[7]), 10);
        break;
*/
...
void ScsiHardDisk::mode_sense_6() {
    ...
    LOG_F(SCSIHD, "%s: Mode_sense_6 page_ctrl:%d page_code:0x%02x sub_page_code:0x%02x alloc_len:%d", name.c_str(), page_ctrl, page_code, sub_page_code, alloc_len);
...
void ScsiHardDisk::read_long_10(uint64_t lba, uint16_t transfer_len, uint8_t cmd_len) {
