...
void ScsiHardDisk::process_command() {
    VLOG_SCOPE_F(loguru::Verbosity_9, "%s: ScsiHardDisk::process_command 0x%X",
...
void ScsiHardDisk::format() {
    ...
    if (this->phase_switch_timer.active)
        LOG_F(WARNING, "%s: phase_switch_timer is already active", this->get_name_and_unit_address().c_str());
