...
HWComponent* ScsiHardDisk::set_property(const std::string &property, const std::string &value, int32_t unit_address) {
    if (property == "hdd_img") {
        if (!value.empty() && !this->img_size && this->override_property(property, value)) {
            ...
            if (this->img_size) {
                LOG_F(INFO, "%s: image size:%lld path:\"%s\"", this->get_path().c_str(), this->img_size, value.c_str());
                return this;
            }
        }
    }
...
int ScsiHardDisk::req_sense(uint16_t alloc_len) {
    ...
    if (this->last_selection_has_atention) {
        LOG_F(SCSIHD, "%s: REQ_SENSE (%d bytes) with ATN LUN = %02x & 7", this->get_name_and_unit_address().c_str(),
              alloc_len, this->last_selection_message);
        lun = this->last_selection_message & 7;
    }
    else {
        LOG_F(SCSIHD, "%s: REQ_SENSE (%d bytes) with NO ATN LUN = %02x >> 5", this->get_name_and_unit_address().c_str(),
              alloc_len, cmd_buf[1]);
        lun = cmd_buf[1] >> 5;
    }
...
void ScsiHardDisk::mode_sense_6() {
    ...
    LOG_F(SCSIHD, "%s: Mode_sense_6 page_ctrl:%d page_code:0x%02x sub_page_code:0x%02x alloc_len:%d", this->get_name_and_unit_address().c_str(), page_ctrl, page_code, sub_page_code, alloc_len);
    ...
    if (page_ctrl == 2) {
        LOG_F(ERROR, "%s: page_ctrl 2 DEFAULT VALUES is not implemented", this->get_name_and_unit_address().c_str());
        this->status = ScsiStatus::CHECK_CONDITION;
        this->sense  = ScsiSense::ILLEGAL_REQ;
        this->asc    = ScsiError::INVALID_CDB;
        this->ascq   = 0;
    ...
    if (!(got_page || page_code == 0x3f)) { // not any of the supported pages or all pages
        ...
        return;
    bad_sub_page:
        LOG_F(WARNING, "%s: unsupported page/subpage %02xh/%02xh in MODE_SENSE_6", this->get_name_and_unit_address().c_str(), page_code, sub_page_code);
        this->status = ScsiStatus::CHECK_CONDITION;
        this->sense  = ScsiSense::ILLEGAL_REQ;
        this->asc    = ScsiError::INVALID_CDB;
        this->ascq   = 0;
...
void ScsiHardDisk::write(uint32_t lba, uint16_t transfer_len, uint8_t cmd_len) {
    ...
    this->post_xfer_action = [this, device_offset]() {
        LOG_F(SCSIHD, "%s: finishing write %lld %d", this->get_name_and_unit_address().c_str(), (long long)device_offset, this->incoming_size);
