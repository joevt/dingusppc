...
void USBHostOHCI::change_one_bar(uint32_t &aperture, uint32_t aperture_size, uint32_t aperture_new, int bar_num) {
    if (aperture != aperture_new) {
        ...
        if (aperture) {
            ...
            SoftwareReset(); // the first thing the driver does is Suspend but you can't Suspend from Reset so we do SoftwareReset to achieve Suspend.
...
uint32_t USBHostOHCI::pci_cfg_read(uint32_t reg_offs, AccessDetails &details)
{
    uint32_t value = PCIDevice::pci_cfg_read(reg_offs, details);
    return value;
}

void USBHostOHCI::pci_cfg_write(uint32_t reg_offs, uint32_t value, AccessDetails &details)
{
    if (reg_offs < 64) {
...
void USBHostOHCI::ohci_wr_HcControlCurrentED(uint32_t v) {
    if (HcOp.HcControl.ControlListEnable) {
        LOG_F(ERROR, "%s:       %-30s from %08x to %08x (should not change while control list is enabled)", this->get_name().c_str(),
...
void USBHostOHCI::ohci_wr_PowerSwitchingMode(PowerSwitchingMode_t v) {
    LOG_F(WARNING, "%s:       %-30s from %s to %s", this->get_name().c_str(),
        "PowerSwitchingMode", get_power_switching_mode_name(HcOp.HcRhDescriptorA.PowerSwitchingMode), get_power_switching_mode_name(v));
...
void USBHostOHCI::ohci_wr_OverCurrentProtectionMode(OverCurrentProtectionMode_t v) {
    LOG_F(WARNING, "%s:       %-30s from %s to %s", this->get_name().c_str(),
        "OverCurrentProtectionMode", get_over_current_protection_mode_name(HcOp.HcRhDescriptorA.OverCurrentProtectionMode), get_over_current_protection_mode_name(v));
...
void USBHostOHCI::ohci_wr_NoOverCurrentProtection(NoOverCurrentProtection_t v) {
    LOG_F(WARNING, "%s:       %-30s from %s to %s", this->get_name().c_str(),
        "NoOverCurrentProtection", get_no_over_current_protection_name(HcOp.HcRhDescriptorA.NoOverCurrentProtection), get_no_over_current_protection_name(v));
...
void USBHostOHCI::NewFrame() {
    if (!UnrecoverableError && !HcOp.HcInterruptStatus.UnrecoverableError && HcOp.HcControl.HostControllerFunctionalState == HCFS_UsbOperational) {
...
void USBHostOHCI::DecrementFrameRemaining(int amount) {
    ...
    if (amount > HcOp.HcFmRemaining.FrameRemaining) {
        frameRemaining = (HcOp.HcFmInterval.FrameInterval + 1 + HcOp.HcFmRemaining.FrameRemaining - (amount % (HcOp.HcFmInterval.FrameInterval + 1))) % (HcOp.HcFmInterval.FrameInterval + 1);
...
void USBHostOHCI::ServiceLists() {
    ...
    while (1) {
        ...
        if (FrameControl.PeriodicListEnable) {
            ...
            // The Host Controller Driver ensures that all Interrupt Endpoint Descriptors are placed on the list in front of any Isochronous Endpoint Descriptors
            uint32_t ed = READ_DWORD_LE_A(&hcca->HccaInterrruptTable[HcOp.HcFmNumber.FrameNumber & 31]);
            while (ed) {
...
void USBHostOHCI::ServiceEd(uint32_t ed, EndpointDescriptor_t &edh, ListType_t list_type) {
    ...
    // FIXME: When the D field of an ED is 10b (IN), the Host Controller may issue an IN token to the specified endpoint
    //        after it determines that HeadP and TailP are not the same. This indicates that a buffer exists for the data and
    //        that input of the endpoint data may occur in parallel with the HC’s access of the TD which defines the memory buffer.
...
void USBHostOHCI::ServiceTdGeneral(EndpointDescriptor_t &edh, uint32_t td, GeneralTransferDescriptor_t &tdh) {
    ...
    bool out = ed0.Direction == DirectionOut || ((ed0.Direction == DirectionFromTD0 || ed0.Direction == DirectionFromTD3) && (gtd0.DirectionPID == DirectionPIDOut || gtd0.DirectionPID == DirectionPIDSetup));
...
bool USBHostOHCI::TransmitPacket(EndpointDescriptor_t &edh, GeneralTransferDescriptor_t &tdh, uint8_t *data1, uint32_t size1, uint8_t *data2, uint32_t size2, uint32_t &bytes_transmitted, bool &ACK, bool &NAK, ConditionCode_t &condition_code)  {
...
bool USBHostOHCI::ReceivePacket(EndpointDescriptor_t &edh, GeneralTransferDescriptor_t &tdh, uint8_t *data1, uint32_t size1, uint8_t *data2, uint32_t size2, uint32_t &bytes_transmitted, bool &ACK, bool &NAK, ConditionCode_t &condition_code)  {
...
void USBHostOHCI::RetireTd(EndpointDescriptor_t &edh, uint32_t td, GeneralTransferDescriptor_t &tdh) {
    ...
    if (gtd0.ErrorCount > 2) {
        ...
        if (gtd0.DelayInterrupt >= DoneQueueInterruptCounter) {
            // In this case, another Transfer Descriptor already on the Done Queue requires an interrupt earlier than the Transfer Descriptor being retired
        }
        else {
            // In this case, the Transfer Descriptor being retired requires an interrupt earlier than all of the Transfer Descriptors currently on the Done Queue
