...
public:
    ...
    HWComponent* get_comp_by_name(const std::string name, bool optional = false, int depth = 0);
    HWComponent* get_comp_by_name_optional(const std::string name);
    HWComponent* get_comp_by_type(HWCompType type, int depth = 0);
    int postinit_devices(int &count);
    int postinit_devices();

    std::string get_path();
    std::string get_name_and_unit_address();
    virtual int32_t parse_self_unit_address_string(std::string unit_address_string);
