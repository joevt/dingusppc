...
void DMAChannel::interpret_cmd() {
    ...
    if (this->cmd_in_progress) {
        VLOG_SCOPE_F(loguru::Verbosity_DBDMA, "%s: interpret_cmd in_progress (ChannelStatus 0x%04x) (%d bytes remaining)", this->get_name().c_str(), this->ch_stat + 0, this->queue_len + 0);
    ...
    VLOG_SCOPE_F(loguru::Verbosity_DBDMA, "%s: interpret_cmd (ChannelStatus 0x%04x):", this->get_name().c_str(), this->ch_stat + 0);
