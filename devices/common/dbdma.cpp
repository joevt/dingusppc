...
void DMAChannel::finish_cmd() {
    ...
    LOG_F(DBDMA, "%s: (ChannelStatus 0x%04x)", this->get_name().c_str(), this->ch_stat);
    if (loguru::Verbosity_DBDMA <= loguru::current_verbosity_cutoff()) {
        dump_program(saved_cmd_ptr, 1, true);
    }
    ...
    if (is_store) {
        if (res.type & RT_MMIO) {
            ...
            LOG_F(DBDMA, "%s: STORE_QUAD 0x%08x.%c = %0*x", this->get_name().c_str(), addr,
                ...
                uint32_t(cmd_arg & ((uint64_t(1) << (xfer_size * 8)) - 1))
            );
        } else {
    ...
    case DMAReg::CMD_PTR_LO:
        if (!(this->ch_stat & CH_STAT_ACTIVE)) { // DBDMA spec 5.5.4 (disagrees with 2.9.1.3)
            ...
            LOG_F(WARNING, "%s: CommandPtrLo should not be set to 0x%X while active (ChannelStatus 0x%04x)",
                this->get_name().c_str(), value, this->ch_stat);
        }
        break;
    case DMAReg::INT_SELECT:
...
void DMAChannel::dump_program(uint32_t cmd_ptr, uint32_t cmd_count, bool is_log) {
    ...
    addresses.insert(cmd_ptr);

    #define STRPRINTF(...) \
        do { \
            if (pos < sizeof(str)) pos += std::snprintf(str + pos, sizeof(str) - pos, __VA_ARGS__); \
        } while(0)

    while(!addresses.empty()) {
        auto node = addresses.extract(addresses.begin());
        cmd_ptr = node.value();
        auto it = std::find_if(ranges.begin(), ranges.end(), [&](const std::pair<uint32_t, uint32_t>& p) {
            return p.first <= cmd_ptr && p.second >= cmd_ptr;
        });
        if (it != ranges.end())
            continue;
        if (!ranges.empty()) {
        ...
        uint32_t cmd_ptr_start = cmd_ptr;
        do {
            fetch_cmd(cmd_ptr, &cmd_struct, &cmd_is_writable);

            int pos = 0;

            DBDMA_Cmd cmd = DBDMA_Cmd(cmd_struct.cmd_key >> 4);
            ...
            switch (cmd) {
                ...
                default                     : STRPRINTF("?cmd%-4d     ", (int)cmd) ; break;
            }
            ...
            if (cmd == DBDMA_Cmd::STORE_QUAD || cmd == DBDMA_Cmd::LOAD_QUAD) {
                ...
                if (cmd_struct.cmd_arg) {
                    if (bits_branch && (
                        ...
                        addresses.insert(cmd_struct.cmd_arg);
                    }
                    else {
                        STRPRINTF("dst:?%08x ", cmd_struct.cmd_arg);
                    }
            ...
            if ((cmd == DBDMA_Cmd::STOP) || (bits_branch == 3))
                break;

            cmd_ptr += 16;
        } while (1);
    
        ranges.push_back(std::pair<uint32_t,uint32_t>(cmd_ptr_start, cmd_ptr));
    } // for addresses
}
