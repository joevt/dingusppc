    ...
    *avail_len = 0;
    ...
    if (this->queue_len) {
        *p_data = this->queue_data;
        ...
        LOG_F(DBDMA, "%s: Will pull_data from 0x%llx : %s", this->get_name().c_str(), (uint64_t)(this->queue_data + 0), hex_string(this->queue_data + 0, *avail_len).c_str());
...
int DMAChannel::push_data(const char* src_ptr, int len) {
    ...
    if (this->queue_len) {
        ...
        //LOG_F(DBDMA, "%s: Did push_data to 0x%llx : %s", this->get_name().c_str(), (uint64_t)this->queue_data, hex_string(src_ptr, len).c_str());
        this->queue_data += len;
        this->res_count  += len;
        this->queue_len  -= len;
    }
