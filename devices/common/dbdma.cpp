...
DMACmd* DMAChannel::fetch_cmd(uint32_t cmd_addr, DMACmd* p_cmd, bool *is_writable) {
    ...
    p_cmd->cmd_key   = cmd_host->cmd_key; // FIXME: read here by audio thread DMAChannel::pull_data; written by CPU thread stw instruction.
...
void DMAChannel::interpret_cmd(const char *from) {
    ...
    if (this->cmd_in_progress) {
        VLOG_SCOPE_F(loguru::Verbosity_DBDMA, "%s: interpret_cmd(%s) in_progress (ChannelStatus 0x%04x) (%d bytes remaining)", this->get_name().c_str(), from, this->ch_stat + 0, this->queue_len + 0);
    ...
    VLOG_SCOPE_F(loguru::Verbosity_DBDMA, "%s: interpret_cmd(%s) (ChannelStatus 0x%04x):", this->get_name().c_str(), from, this->ch_stat + 0);
    ...
    case DBDMA_Cmd::INPUT_LAST:
        ...
        if (this->queue_len) {
            ...
            this->res_count  = 0;
            LOG_F(DBDMA, "%s: Will transfer %d bytes %s 0x%08x (host:0x%llx)", this->get_name().c_str(), this->queue_len + 0, this->cur_cmd > DBDMA_Cmd::OUTPUT_LAST ? "to" : "from", cmd_struct.address, (uint64_t)(this->queue_data + 0));
...
void DMAChannel::finish_cmd() {
    ...
    uint32_t save_cmd_ptr = this->cmd_ptr;
    ...
    // get command code
    this->cur_cmd = cmd_desc[3] >> 4; // FIXME: read here by audio thread DMA_Channel::pull_data .. interpret_cmd; written by CPU thread stw instruction
...
void DMAChannel::reg_write(uint32_t offset, uint32_t value, int size) {
    ...
    case DMAReg::CH_CTRL:
        ...
        LOG_F(DBDMA, "%s: ChannelStatus mask = 0x%04X value = 0x%04X old = 0x%04X new = 0x%04X", this->get_name().c_str(), mask, value & 0xffff, old_stat, new_stat);
        if ((CH_STAT_WAKE | CH_STAT_FLUSH) & mask & ~value & old_stat) {
            LOG_F(ERROR, "%s: Attempt to clear command bits 0x%04x", this->get_name().c_str(), mask & (CH_STAT_WAKE | CH_STAT_FLUSH) & ~value);
        }
        if ((CH_STAT_BT | CH_STAT_ACTIVE | CH_STAT_DEAD) & mask & (~value ^ ~old_stat)) {
            LOG_F(ERROR, "%s: Attempt to change status bits 0x%04x", this->get_name().c_str(), (CH_STAT_BT | CH_STAT_ACTIVE | CH_STAT_DEAD) & mask & (~value ^ ~old_stat));
        }
        ...
        if (new_stat & CH_STAT_FLUSH) {
            if (this->cur_cmd <= DBDMA_Cmd::INPUT_LAST &&
                ...
                !(this->ch_stat & CH_STAT_DEAD)) {
                this->flush_cb();
                LOG_F(DBDMA, "%s: Did flush_cb.", this->get_name().c_str());
            } else {
    ...
    case DMAReg::CMD_PTR_LO:
        if (!(this->ch_stat & CH_STAT_RUN) && !(this->ch_stat & CH_STAT_ACTIVE)) { // DBDMA spec 5.5.4 (disagrees with 2.9.1.3)
            ...
            this->cmd_ptr = value;
            LOG_F(WARNING, "%s: CommandPtrLo should not be set to 0x%X while running or active (ChannelStatus 0x%04x)",
    ...
    if (this->queue_len) {
        *p_data = this->queue_data;
        ...
        LOG_F(DBDMA, "%s: Will pull %d bytes from 0x%llx (next:0x%llx, count:%d, queue:%d) : %s",
            this->get_name().c_str(), *avail_len, (uint64_t)(*p_data), (uint64_t)(this->queue_data + 0),
            this->res_count + 0,
            this->queue_len + 0,
            hex_string(*p_data, *avail_len).c_str()
        );
...
int DMAChannel::push_data(const char* src_ptr, int len) {
    ...
    if (this->queue_len) {
        ...
        this->res_count  += len;
        this->queue_len  -= len;
        if ((uint64_t)(this->queue_data - len) < 5 || (uint64_t)(this->queue_data + 0) < 5)
        LOG_F(DBDMA, "%s: Did push %d bytes to 0x%llx (next:0x%llx, count:%d, queue:%d) : %s",
            this->get_name().c_str(), len, (uint64_t)(this->queue_data - len), (uint64_t)(this->queue_data + 0),
            this->res_count + 0,
            this->queue_len + 0,
            hex_string((uint8_t *)src_ptr, len).c_str()
        );
    }
