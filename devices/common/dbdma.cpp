...
uint8_t DMAChannel::interpret_cmd() {
    ...
    case DBDMA_Cmd::INPUT_LAST:
        ...
        if (this->queue_len) {
            ...
            this->res_count  = 0;
            LOG_F(DBDMA, "%s: Will transfer %d bytes %s 0x%08x (host:0x%llx)", this->get_name().c_str(), this->queue_len, this->cur_cmd > DBDMA_Cmd::OUTPUT_LAST ? "to" : "from", cmd_struct.address, (uint64_t)(this->queue_data));
    ...
    *avail_len = 0;
    ...
    if (this->queue_len) {
        *p_data = this->queue_data;
        ...
        LOG_F(DBDMA, "%s: Will pull_data from 0x%llx : %s", this->get_name().c_str(), (uint64_t)this->queue_data, hex_string(this->queue_data, *avail_len).c_str());
