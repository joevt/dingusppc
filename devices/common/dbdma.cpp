...
void DMAChannel::interpret_cmd(const char *from) {
    ...
    if (this->cmd_in_progress) {
        VLOG_SCOPE_F(loguru::Verbosity_DBDMA, "%s: interpret_cmd(%s) in_progress (ChannelStatus 0x%04x) (%d bytes remaining)",
            this->get_name().c_str(), from, this->ch_stat + 0, this->queue_len + 0);
    ...
    VLOG_SCOPE_F(loguru::Verbosity_DBDMA, "%s: interpret_cmd(%s) (ChannelStatus 0x%04x):",
        this->get_name().c_str(), from, this->ch_stat + 0);
...
uint32_t DMAChannel::reg_read(uint32_t offset, int size) {
    ...
    if ((offset & 3) == 0 & size == 4) [[likely]] {
        return BYTESWAP_32(value);
    }
    if (size < 0)
        return value;
    uint32_t value2 = 0;
    if ((offset & 3) + size > 4) [[unlikely]] {
...
void DMAChannel::reg_write(uint32_t offset, uint32_t value, int size) {
    ...
    case DMAReg::CH_CTRL:
        ...
        if ((CH_STAT_WAKE | CH_STAT_FLUSH) & mask & ~data & old_stat) {
            LOG_F(ERROR, "%s: Attempt to clear command bits 0x%04x",
                this->get_name().c_str(), mask & (CH_STAT_WAKE | CH_STAT_FLUSH) & ~data);
        }
        if ((CH_STAT_BT | CH_STAT_ACTIVE | CH_STAT_DEAD) & mask & (~data ^ ~old_stat)) {
            LOG_F(ERROR, "%s: Attempt to change status bits 0x%04x",
                this->get_name().c_str(), (CH_STAT_BT | CH_STAT_ACTIVE | CH_STAT_DEAD) & mask & (~data ^ ~old_stat));
        }
        ...
        if (mask & CH_STAT_FLUSH) {
            ...
            if (data & CH_STAT_FLUSH) {
                if (
                    ...
                    LOG_F(DBDMA, "%s: Attempt to flush when not doing I/O",
                        this->get_name().c_str());
                }
            } else {
    ...
    if (this->queue_len) {
        *p_data = this->queue_data;
        ...
        LOG_F(DBDMA, "%s: Will pull %d bytes from 0x%llx (next:0x%llx, count:%d, queue:%d) : %s",
            ...
            this->res_count + 0, this->queue_len + 0, hex_string(*p_data, *avail_len).c_str()
        );
    ...
    if (this->queue_len) {
        ...
        if ((uint64_t)(this->queue_data - len) < 5 || (uint64_t)(this->queue_data + 0) < 5) {
            LOG_F(DBDMA, "%s: Did push %d bytes to 0x%llx (next:0x%llx, count:%d, queue:%d) : %s",
                ...
                this->res_count + 0, this->queue_len + 0, hex_string((uint8_t *)src_ptr, len).c_str()
            );
        }
    }
...
void DMAChannel::dump_program(uint32_t cmd_ptr, uint32_t cmd_count) {
    ...
    do {
        ...
        uint8_t key_reserved = cmd_struct.cmd_key & 8;
