...
{
    LOG_F(SUPERDRIVE, "%s: command %-17s addr=0x%X, value=%d", this->get_name().c_str(), get_command_name(addr).c_str(), addr, value);
...
{
    LOG_F(READWRITE, "%s: get_sector_data_ptr track:%d head:%d sector:%d", this->get_name().c_str(), this->cur_track, this->cur_head, sector_num);
