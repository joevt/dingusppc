    ...
    case Swim3Reg::Phase:
        ...
        if (this->phase_lines & 8) { // CA3 aka LSTRB high -> sending a command to the drive
            ...
            if (this->selected_drive) {
                ...
                LOG_F(ERROR, "SWIM3: command %-17s addr=0x%X, value=%d; no drive selected yet", MacSuperdrive::get_command_name(command_addr).c_str(), command_addr, val);
            ...
            if (this->selected_drive)
                ...
                LOG_F(ERROR, "SWIM3: status %-13s 0x%X; no drive selected yet", MacSuperdrive::get_status_name(status_addr).c_str(), status_addr);
        }
        break;
    case Swim3Reg::Setup:
...
void Swim3Ctrl::disk_access()
{
    ...
    case SWIM3_ADDR_MARK_SEARCH:
        ...
        LOG_F(SWIM3, "SWIM3: set side:%d track:%d valid:%d sector:%d format:%d", this->cur_track >> 7, this->cur_track & 0x7F, this->cur_sector >> 7, this->cur_sector & 0x7F, this->format);
...
static const PropMap Swim3_Properties = {
    ...
    {"fdd_drives",
        new IntProperty(1, vector<uint32_t>({0, 1, 2}))},
};
