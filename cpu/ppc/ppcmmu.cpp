...
#ifdef LOG_TAG
        if (tag == 0x0030b000) {
            int mode = (pCurDTLB2 == &dtlb2_mode1[0]) ? 1 : pCurDTLB2 == &dtlb2_mode2[0] ? 2 : pCurDTLB2 == &dtlb2_mode3[0] ? 3 : -1;
            if (refillcountdown > 0 || lastmode != mode || lasttag != tlb_entry->tag || lastptag != tlb_entry->phys_tag || lastflags != tlb_entry->flags) {
...
uint8_t *mmu_translate_imem(uint32_t vaddr, uint32_t *paddr)
{
    ...
    if (verify) {
        ...
        if (tlb2_entry->phys_tag != savedphys) {
            LOG_F(ERROR, "mmu_translate_imem; phystag mismatch address:0x%08x tag:0x%08x phys:0x%08x correctedphys:%08x", vaddr, tag, savedphys, tlb2_entry->phys_tag);
            dump_backtrace();
        }
    }
#endif
    ...
    for (auto &tlb_el : tlb) {
        if (tlb_el.tag != TLB_INVALID_TAG && tlb_el.flags & type) {
            ...
            if (tlb_el.tag == 0x0030b000) {
                ...
                if (tlb_el.phys_tag == 0x01e0b000 || lastmode2 != mode || lasttag2 != tlb_el.tag || lastptag2 != tlb_el.phys_tag) {
...
template <class T>
static T read_unaligned(uint32_t opcode, uint32_t guest_va, uint8_t *host_va, bool needs_swap, bool munged);
template <class T>
static void write_unaligned(uint32_t opcode, uint32_t guest_va, uint8_t *host_va, T value, bool needs_swap, bool munged);
...
inline T mmu_read_vmem(uint32_t opcode, uint32_t guest_va)
{
...
#ifdef LOG_TAG
            if (tag == 0x0030b000) {
                int mode = (pCurDTLB1 == &dtlb1_mode1[0]) ? 1 : pCurDTLB1 == &dtlb1_mode2[0] ? 2 : pCurDTLB1 == &dtlb1_mode3[0] ? 3 : -1;
    ...
    if (verify) {
        ...
        tlb2_entry = dtlb2_refill(guest_va, 0);
        if (tlb2_entry->phys_tag != savedphys) {
            LOG_F(ERROR, "mmu_read_vmem; phystag mismatch address:0x%08x tag:0x%08x phys:0x%08x correctedphys:%08x", guest_va, tag, savedphys, tlb2_entry->phys_tag);
            dump_backtrace();
        }
    }
#endif
    ...
    if (guest_va >= 0x30B404 && guest_va < 0x30B408) {
        if (!watch_point_dma || *watch_point_dma != 0x12000000) {
            if ((uint32_t*)((uint64_t)(host_va) & ~3) != watch_point_dma) {
                LOG_F(ERROR, "mmu_read_vmem; reading from cpu_type host_va changed from 0x%llx to 0x%llx", (uint64_t)watch_point_dma, (uint64_t)host_va & ~3);
                watch_point_dma = (uint32_t*)((uint64_t)(host_va) & ~3);
            }
        }
        if (got_watch_point_value) {
            LOG_F(ERROR, "mmu_read_vmem; reading from cpu_type value:0x%08x size:%d guest_pa:0x%08x host_va:0x%llx", READ_DWORD_BE_A((uint64_t)host_va & ~3), (int)sizeof(T), guest_pa, (uint64_t)host_va);
            dump_backtrace();
        }
    }
#endif
    ...
    if (sizeof(T) > 1 && (guest_va & (sizeof(T) - 1))) {
        if (munged)
            guest_va = mem_munge_address<T>(guest_va);
        return read_unaligned<T>(opcode, guest_va, host_va, needs_swap, munged);
    }
...
inline void mmu_write_vmem(uint32_t opcode, uint32_t guest_va, T value)
{
...
#ifdef LOG_TAG
            if (tag == 0x0030b000) {
                LOG_F(ERROR, "mmu_write_vmem; perform full page address translation to update PTE.C mode:%d tag:0x%08x phys:0x%08x",
                    (pCurDTLB1 == &dtlb1_mode1[0]) ? 1 : pCurDTLB1 == &dtlb1_mode2[0] ? 2 : pCurDTLB1 == &dtlb1_mode3[0] ? 3 : -1,
    ...
    if (guest_va >= 0x30B404 && guest_va < 0x30B408) {
        if (!watch_point_dma || value == 0x12000000) {
            if ((uint32_t*)((uint64_t)(host_va) & ~3) != watch_point_dma) {
                LOG_F(ERROR, "mmu_write_vmem; writing to cpu_type host_va changed from 0x%llx to 0x%llx", (uint64_t)watch_point_dma, (uint64_t)host_va & ~3);
                watch_point_dma = (uint32_t*)((uint64_t)(host_va) & ~3);
            }
            got_watch_point_value = true;
        }
        LOG_F(ERROR, "mmu_write_vmem; writing to cpu_type value:0x%08llx size:%d guest_pa:0x%08x host_va:0x%llx", (uint64_t)value, (int)sizeof(T), guest_pa, (uint64_t)host_va);
        dump_backtrace();
    }
#endif
    ...
    if (sizeof(T) > 1 && (guest_va & (sizeof(T) - 1))) {
        ...
        if (munged)
            guest_va = mem_munge_address<T>(guest_va);
        write_unaligned<T>(opcode, guest_va, host_va, value, needs_swap, munged);
        return;
    }
...
template uint16_t read_unaligned<uint16_t>(uint32_t opcode, uint32_t guest_va, uint8_t* host_va, bool needs_swap, bool munged);
template uint32_t read_unaligned<uint32_t>(uint32_t opcode, uint32_t guest_va, uint8_t* host_va, bool needs_swap, bool munged);
template uint64_t read_unaligned<uint64_t>(uint32_t opcode, uint32_t guest_va, uint8_t* host_va, bool needs_swap, bool munged);
...
template void write_unaligned<uint16_t>(uint32_t opcode, uint32_t guest_va, uint8_t *host_va, uint16_t value, bool needs_swap, bool munged);
template void write_unaligned<uint32_t>(uint32_t opcode, uint32_t guest_va, uint8_t *host_va, uint32_t value, bool needs_swap, bool munged);
template void write_unaligned<uint64_t>(uint32_t opcode, uint32_t guest_va, uint8_t *host_va, uint64_t value, bool needs_swap, bool munged);
