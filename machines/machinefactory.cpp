...
void MachineFactory::apply_configs()
{
    ...
    for (auto& config : configs) {
        ...
        if (!bus_obj) {
            LOG_F(ERROR, "%s device path \"%s\" not found", config.first.c_str(), the_config.c_str());
            continue;
        }
        ...
        if (!device_created)
            LOG_F(ERROR, "%s device path \"%s\" does not belong to a SCSI or ATA bus", config.first.c_str(), the_config.c_str());
    }
}
...
string MachineFactory::machine_name_from_rom(char *rom_data, size_t rom_size, bool fix_checksums) {
    ...
    if (!has_nw_config && !(date > 0x19980710 && date <= 0x20051231)) {
        date = READ_DWORD_BE_A(&rom_data[12]);
    }

    if (has_nw_config || (date >= 0x19980101 && date <= 0x20051213)) {
        is_nw = true;
        if (has_nw_config || date > 0x19980710)
