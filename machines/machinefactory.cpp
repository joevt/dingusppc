            ...
            [&](HWComponent *it, int depth) {
                ...
                for (auto& s : it->device_settings) {
                    if (s.second->value_commandline == s.second->value_not_inited) {
                        LOG_F(INFO, "Defaulting %s property \"%s\"", it->get_name_and_unit_address().c_str(), s.first.c_str());
...
HWComponent* MachineFactory::set_property(const std::string &property, const std::string &value)
{
    ...
    for (int i = int(config_stack.size()) - 1; i >= 0; i--) {
        ...
        if (cs->stack_item_type == ConfigStackItem::HWC) {
            ...
            // VLOG_SCOPE_F(loguru::Verbosity_INFO, "checking type 2 %s%s", cs->hwc->get_path().c_str(), cs->hwc->get_child_unit_address_string(cs->unit_address).c_str());
...
void MachineFactory::print_settings(const PropMap& prop_map, PropScope scope, int /*indent*/, string path, string device, std::set<string> *properties)
{
    string help;
...
void MachineFactory::register_device_settings(const std::string& name)
{
    ...
    register_settings(name, dev.properties);
    for (auto& d : dev.subdev_list) {
