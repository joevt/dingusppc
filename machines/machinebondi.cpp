...
static std::unique_ptr<HWComponent> create() {
    ...
    if (machine && 0 == machine->initialize_bondi())
        return std::unique_ptr<MachineBondi>(machine);
    if (machine)
        delete machine;
    return nullptr;
}

int initialize_bondi()
{
...
static const DeviceDescription MachineBondi_descriptor = {
    MachineBondi::create, bondi_devices, bondi_settings, HWCompType::MACHINE, "iMac G3 Bondi Blue"
};
